#!/usr/bin/env python3
"""A kernel-fragmented TUN datagram traverses local assembly and the real stack."""
import socket
import time
import subprocess
import json
...
def main():
    ...
    payload = bytes((i * 31 + 7) & 255 for i in range(8192))
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as server:
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as client:
                ...
                for _ in range(3):
                    client.send(payload)
                    assert client.recv(65535) == payload, "fragmented roundtrip changed payload"
        finally:
