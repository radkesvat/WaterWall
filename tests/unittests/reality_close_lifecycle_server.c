...
static void serverInjectDestinationDownstreamCallbacks(server_lifecycle_context_t *context, line_t *l)
{
...
static void serverPrevPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    requireServer(sbufGetLength(buf) == (uint32_t) context->expected_alert_body_len + 5U &&
                      record[0] == context->expected_alert_type && record[1] == 0x03 && record[2] == 0x03 &&
    ...
    requireServer(realityV2BuildRecordDescriptor(
                      ls->tls_capture.binding.tls_version, &ls->record_profile, kRealityV2RecordKindAlert, &descriptor),
    ...
    requireServer(realityV2BuildRecordAad(&descriptor,
                                          ...
                                          aad,
                                          &aad_len) &&
                      wCryptoChaCha20Poly1305Decrypt(
                          inner, sizeof(inner), ciphertext, ciphertext_len, aad, aad_len, nonce, ls->s2c_key) ==
                          kWCryptoOk,
                  "failed to decrypt server final alert");
    uint32_t payload_offset = 0;
    uint32_t payload_len    = 0;
    requireServer(realityV2ValidateInnerPlaintext(
                      &descriptor, inner, ciphertext_len - kRealityV2TagSize, &payload_offset, &payload_len) &&
                      realityV2ParseAlert(inner + payload_offset, payload_len, &context->observed_alert),
...
static void serverNextPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    requireServer(sbufGetLength(buf) == 1 && payload[0] == 0x5a, "server delivered the wrong application payload");
...
static wcrypto_status_t serverControlDecrypt(void *context, unsigned char *dst, size_t dst_capacity,
...
static void serverHandoffPrevPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    if (ls->mode != kRealityServerModeHandoffAwaitConfirm)
    {
        ...
        memoryCopy(context->cover_downstream + context->cover_downstream_bytes, sbufGetRawPtr(buf), len);
    ...
    uint32_t                       payload_offset = 0;
    uint32_t                       payload_len    = 0;
    requireServer(ls->handoff_ack_sent && ls->destination_downstream_cutoff && ls->s2c_send_seq == 1 &&
                      realityV2BuildRecordDescriptor(
                          kRealityV2Tls13, &ls->record_profile, kRealityV2RecordKindHandoffAck, &descriptor) &&
                      realityV2TryDecryptExpectedRecord(&descriptor,
                                                        ...
                                                        &payload_offset,
                                                        &payload_len) &&
                      realityV2ParseControl(kRealityV2RecordKindHandoffAck, plaintext + payload_offset, payload_len),
...
static void serverFixtureSetFailureMode(server_lifecycle_fixture_t *fixture, bool kill_on_payload,
                                        bool kill_on_next_finish)
{
...
static void serverFixtureDestroy(server_lifecycle_fixture_t *fixture)
{
    ...
    {
        requireServer(! lineIsAlive(fixture->line) && fixture->line_pool->len == fixture->line_pool_available,
...
static realityserver_lstate_t *serverFixtureMoveLineToOwnerPool(server_lifecycle_fixture_t *fixture)
{
...
static void runServerScenario(void (*action)(tunnel_t *, line_t *), const char *expected, bool kill_on_payload,
                              bool kill_on_next_finish)
{
...
static sbuf_t *buildServerInboundRecord(server_lifecycle_fixture_t *fixture, uint16_t tls_version,
                                        const reality_v2_record_profile_t *profile, uint8_t record_kind,
                                        const uint8_t *payload, uint32_t payload_len, uint64_t sequence)
{
    realityserver_lstate_t        *ls = lineGetState(fixture->line, fixture->reality);
    reality_v2_record_descriptor_t descriptor;
    reality_v2_record_layout_t     layout;
    requireServer(realityV2BuildRecordDescriptor(tls_version, profile, record_kind, &descriptor) &&
    ...
    requireServer(
        realityV2BuildInnerPlaintext(&descriptor, payload, payload_len, ciphertext, layout.inner_plaintext_len),
    ...
    requireServer(realityV2BuildRecordAad(&descriptor,
                                          ...
                                          aad,
                                          &aad_len) &&
                      wCryptoChaCha20Poly1305Encrypt(ciphertext,
...
static sbuf_t *buildServerInboundControl(server_lifecycle_fixture_t *fixture, uint8_t record_kind, uint64_t sequence)
{
    const uint8_t padding[kRealityV2ControlMinPadding] = {0xa1, 0xb2, 0xc3};
    uint8_t       payload[kRealityV2ControlMinPayload];
    requireServer(realityV2SerializeControl(record_kind, padding, sizeof(padding), payload, sizeof(payload)),
                  "failed to serialize server handoff control fixture");
    realityserver_lstate_t *ls = lineGetState(fixture->line, fixture->reality);
    return buildServerInboundRecord(
        fixture, kRealityV2Tls13, &ls->record_profile, record_kind, payload, sizeof(payload), sequence);
}

static void prepareServerPendingProfile(server_lifecycle_fixture_t *fixture, uint16_t tls_version,
                                        const reality_v2_record_profile_t *profile)
{
    ...
    ls->tls13_pre_request_cover_records_remaining =
        tls_version == kRealityV2Tls13 ? kRealityServerTls13PreRequestCoverRecordAllowance : 0;
...
static uint32_t serverTlsRecordLength(const uint8_t *record, size_t remaining, const char *message)
{
...
static sbuf_t *buildServerTls13GenericCandidate(server_lifecycle_fixture_t *fixture, uint32_t body_len, uint8_t fill)
{
...
static void runFirstFatalAuthorization(uint16_t tls_version, const reality_v2_record_profile_t *profile)
{
    server_lifecycle_fixture_t fixture;
    serverFixtureInitialize(&fixture);
    ...
    sbuf_t *record =
        buildServerInboundRecord(&fixture, tls_version, profile, kRealityV2RecordKindAlert, alert, sizeof(alert), 0);
...
static void runServerFragmentedAlert(uint16_t tls_version, const reality_v2_record_profile_t *profile)
{
    ...
    sbuf_t *whole = buildServerInboundRecord(
        &template_fixture, tls_version, profile, kRealityV2RecordKindAlert, alert, sizeof(alert), 0);
    ...
    for (uint32_t split = 1; split < whole_len; ++split)
    {
        ...
        reality_v2_record_descriptor_t descriptor;
        reality_v2_record_layout_t     layout;
        requireServer(realityV2BuildRecordDescriptor(tls_version, profile, kRealityV2RecordKindAlert, &descriptor) &&
                          realityV2CalculateDescriptorLayout(&descriptor, sizeof(alert), &layout),
                      "failed to restore fragmented server alert shape");
        fixture.context.expected_alert_type     = descriptor.outer_content_type;
        fixture.context.expected_alert_body_len = (uint16_t) layout.wire_body_len;
        requireServer(
            realityserverProcessUpstream(fixture.reality, fixture.line, serverBufferFromBytes(&fixture, record, split)),
            "server rejected an incomplete alert fragment");
        requireServer(fixture.context.events_len == 0, "server emitted callbacks before the alert was complete");
        requireServer(
            ! realityserverProcessUpstream(
                fixture.reality, fixture.line, serverBufferFromBytes(&fixture, record + split, whole_len - split)),
            "server did not terminate after a complete fragmented alert");
        requireServer(strcmp(fixture.context.events, "IUD") == 0, "server fragmented alert callback ordering mismatch");
        serverFixtureDestroy(&fixture);
    }
}

static void testServerCoalescedRecords(uint16_t tls_version, const reality_v2_record_profile_t *profile,
                                       bool alert_first)
{
    server_lifecycle_fixture_t fixture;
    serverFixtureInitialize(&fixture);
    ...
    sbuf_t *first =
        buildServerInboundRecord(&fixture,
                                 tls_version,
                                 profile,
                                 alert_first ? kRealityV2RecordKindAlert : kRealityV2RecordKindApplicationData,
    ...
    sbuf_t *second =
        buildServerInboundRecord(&fixture,
                                 tls_version,
                                 profile,
                                 alert_first ? kRealityV2RecordKindApplicationData : kRealityV2RecordKindAlert,
    ...
    requireServer(! realityserverProcessUpstream(
                      fixture.reality, fixture.line, serverBufferFromBytes(&fixture, combined, combined_len)),
...
static void testServerTerminalReentry(void)
{
    ...
    requireServer(strcmp(fixture.context.events, "PD") == 0, "server terminal re-entry emitted duplicate callbacks");
...
static void testServerUnauthenticatedCloseModes(void)
{
    ...
    for (size_t i = 0; i < sizeof(modes) / sizeof(modes[0]); ++i)
    {
        ...
        ls->protected_init_sent    = false;
        realityserverHandleDownstreamFinish(fixture.reality, fixture.line);
        requireServer(strcmp(fixture.context.events, "D") == 0, "pre-authorization close synthesized a Reality alert");
        serverFixtureDestroy(&fixture);
    }
    ...
    requireServer(fixture.context.events_len == 0, "destination teardown synthesized a Reality alert or Finish");
    realityserverLinestateDestroy(ls);
    serverFixtureDestroy(&fixture);
}
...
static void testServerProgressiveVisitorPrefixForwarding(void)
{
    ...
    for (uint8_t parser_complete = 0; parser_complete <= 1; ++parser_complete)
    {
        for (size_t i = 0; i < sizeof(invalid_prefixes) / sizeof(invalid_prefixes[0]); ++i)
        {
            ...
            {
                requireServer(
                    realityserverProcessUpstream(fixture.reality,
                                                 fixture.line,
                ...
                {
                    requireServer(ls->mode == kRealityServerModePending && fixture.context.events_len == 0,
                                  "server classified a plausible visitor prefix too early");
                }
            }

            requireServer(ls->mode == kRealityServerModeVisitor && fixture.context.destination_upstream_records == 1 &&
...
static void testServerPendingPrefixFlushesBeforeFinish(void)
{
    ...
    for (uint32_t length = 1; length <= sizeof(plausible_prefix); ++length)
    {
        ...
        requireServer(realityserverProcessUpstream(
                          fixture.reality, fixture.line, serverBufferFromBytes(&fixture, plausible_prefix, length)) &&
                          ls->mode == kRealityServerModePending && bufferstreamGetBufLen(&ls->read_stream) == length &&
    ...
    realityserverHandleUpstreamFinish(fixture.reality, fixture.line);
    requireServer(fixture.context.destination_upstream_records == 0 && strcmp(fixture.context.events, "F") == 0,
...
static void testServerPendingFinishReentryAndOwnership(void)
{
    ...
    requireServer(realityserverProcessUpstream(
                      fixture.reality, fixture.line, serverBufferFromBytes(&fixture, prefix, sizeof(prefix))),
                  "server rejected the re-entrant Finish prefix fixture");
    fixture.context.finish_on_destination_payload = true;
    realityserverHandleUpstreamFinish(fixture.reality, fixture.line);
    requireServer(lineIsAlive(fixture.line) && fixture.context.destination_upstream_records == 1 &&
                      strcmp(fixture.context.events, "T") == 0,
    ...
    realityserverHandleUpstreamFinish(fixture.reality, fixture.line);
    requireServer(fixture.context.destination_callbacks_injected == 5 && fixture.context.events_len == 1 &&
                      strcmp(fixture.context.events, "T") == 0,
    ...
    realityserverHandleUpstreamFinish(fixture.reality, fixture.line);
    requireServer(fixture.context.destination_upstream_records == 0 && fixture.context.events_len == 0,
...
static void testServerSensitiveModesDoNotFlushPendingBytes(void)
{
    ...
    for (size_t i = 0; i < sizeof(modes) / sizeof(modes[0]); ++i)
    {
        ...
        requireServer(fixture.context.destination_upstream_records == 0 &&
                          strcmp(fixture.context.events, modes[i] == kRealityServerModeAuthorized ? "U" : "F") == 0,
    ...
    realityserverFailAuthenticated(fixture.reality, fixture.line);
    requireServer(fixture.context.destination_upstream_records == 0 && strcmp(fixture.context.events, "FD") == 0,
    ...
    realityserverHandleDownstreamFinish(fixture.reality, fixture.line);
    requireServer(fixture.context.destination_upstream_records == 0 && strcmp(fixture.context.events, "D") == 0,
...
static void testServerTlsRecordBoundaryTracker(void)
{
    ...
    for (size_t split = 1; split < sizeof(first_record); ++split)
    {
        ...
        size_t consumed = 0;
        requireServer(realityserverTlsRecordBoundaryTrackerFeed(&tracker, first_record, split, false, &consumed) &&
                          consumed == split && ! realityserverTlsRecordBoundaryTrackerIsAtBoundary(&tracker),
                      "server boundary tracker lost a fragmented record prefix");
        requireServer(realityserverTlsRecordBoundaryTrackerFeed(
                          &tracker, first_record + split, sizeof(first_record) - split, true, &consumed) &&
    ...
    realityserverTlsRecordBoundaryTrackerInitialize(&tracker);
    size_t consumed = 0;
    requireServer(realityserverTlsRecordBoundaryTrackerFeed(&tracker, coalesced, sizeof(coalesced), true, &consumed) &&
                      consumed == sizeof(first_record) && realityserverTlsRecordBoundaryTrackerIsAtBoundary(&tracker),
                  "server boundary tracker did not split at the first complete record");
    requireServer(realityserverTlsRecordBoundaryTrackerFeed(
                      &tracker, coalesced + consumed, sizeof(coalesced) - consumed, false, &consumed) &&
                      consumed == sizeof(second_record) && realityserverTlsRecordBoundaryTrackerIsAtBoundary(&tracker),
                  "server boundary tracker did not resume after a cutoff split");

    const uint8_t invalid_header[] = {0x18, 0x03, 0x03, 0x00, 0x00};
    realityserverTlsRecordBoundaryTrackerInitialize(&tracker);
    requireServer(! realityserverTlsRecordBoundaryTrackerFeed(
                      &tracker, invalid_header, sizeof(invalid_header), false, &consumed) &&
                      tracker.failed,
...
static void testServerTls13AuthenticatedHandoff(void)
{
    ...
    realityserverTunnelDownStreamPayload(
        fixture.reality, fixture.line, serverBufferFromBytes(&fixture, cover_record, first_cover_part));
    ...
    sbuf_t       *application_before_request = buildServerInboundRecord(&fixture,
                                                                  kRealityV2Tls13,
                                                                  &profile,
                                                                  ...
                                                                  0);
    requireServer(realityserverProcessUpstream(fixture.reality, fixture.line, application_before_request) &&
    ...
    requireServer(
        realityserverProcessUpstream(fixture.reality,
                                     fixture.line,
                                     buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0)) &&
            ls->mode == kRealityServerModeHandoffAwaitBoundary && ls->handoff_request_authenticated &&
            ! ls->handoff_ack_sent && ls->c2s_recv_seq == 1 && ! ls->protected_init_sent,
    ...
    uint8_t  final_cover_callback[(sizeof(cover_record) - first_cover_part) + sizeof(suppressed_record)];
    ...
    memoryCopy(final_cover_callback + final_prefix_len, suppressed_record, sizeof(suppressed_record));
    realityserverTunnelDownStreamPayload(
        fixture.reality,
        fixture.line,
        serverBufferFromBytes(&fixture, final_cover_callback, sizeof(final_cover_callback)));
    requireServer(ls->mode == kRealityServerModeHandoffAwaitConfirm && ls->handoff_ack_sent &&
                      ls->destination_downstream_cutoff && ls->s2c_send_seq == 1 &&
    ...
    realityserverTunnelDownStreamPayload(
        fixture.reality, fixture.line, serverBufferFromBytes(&fixture, suppressed_record, sizeof(suppressed_record)));
    ...
    requireServer(realityserverProcessUpstream(fixture.reality,
                                               ...
                                               buildServerInboundRecord(&fixture,
                                                                        ...
                                                                        sizeof(early_application),
                                                                        1)) &&
                      ls->mode == kRealityServerModeHandoffAwaitConfirm && ls->c2s_recv_seq == 1 &&
                      fixture.context.destination_upstream_records == 2 && strcmp(fixture.context.events, "cTcKT") == 0,
                  "server did not route late cover TLS output to the destination");

    requireServer(
        realityserverProcessUpstream(fixture.reality,
                                     fixture.line,
                                     buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffConfirm, 1)) &&
            ls->mode == kRealityServerModeAuthorized && ls->c2s_recv_seq == 2 && ls->s2c_send_seq == 1 &&
            ls->destination_up_finished && ls->protected_init_sent && strcmp(fixture.context.events, "cTcKTFI") == 0,
...
static void testServerTls13ReentrantRequestCutsOffCoalescedSuffix(void)
{
    ...
    fixture.context.request_on_cover_payload =
        buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0);

    realityserverTunnelDownStreamPayload(
        fixture.reality, fixture.line, serverBufferFromBytes(&fixture, coalesced, sizeof(coalesced)));

    requireServer(ls->mode == kRealityServerModeHandoffAwaitConfirm && ls->handoff_request_authenticated &&
                      ls->handoff_ack_sent && ls->destination_downstream_cutoff && ls->c2s_recv_seq == 1 &&
                      ls->s2c_send_seq == 1 && fixture.context.handoff_ack_records == 1 &&
                      fixture.context.cover_downstream_bytes == sizeof(first_record) &&
                      memoryEqual(fixture.context.cover_downstream, first_record, sizeof(first_record)) &&
...
static void testServerTls13NestedDestinationPayloadCompletesBoundary(void)
{
    ...
    uint8_t nested_payload[(sizeof(first_record) - first_part_len) + sizeof(suppressed_record)];
    memoryCopy(nested_payload, first_record + first_part_len, sizeof(first_record) - first_part_len);
    memoryCopy(nested_payload + sizeof(first_record) - first_part_len, suppressed_record, sizeof(suppressed_record));

    fixture.context.request_on_cover_payload =
        buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0);
    fixture.context.destination_payload_on_cover_payload =
        serverBufferFromBytes(&fixture, nested_payload, sizeof(nested_payload));

    realityserverTunnelDownStreamPayload(
        fixture.reality, fixture.line, serverBufferFromBytes(&fixture, first_record, first_part_len));
    ...
    requireServer(fixture.context.request_on_cover_payload == NULL &&
                      ...
                      ls->mode == kRealityServerModeHandoffAwaitConfirm && ls->handoff_request_authenticated &&
                      ls->handoff_ack_sent && ls->destination_downstream_cutoff && ls->c2s_recv_seq == 1 &&
                      ls->s2c_send_seq == 1 && fixture.context.handoff_ack_records == 1 &&
                      fixture.context.cover_downstream_bytes == sizeof(first_record) &&
                      memoryEqual(fixture.context.cover_downstream, first_record, sizeof(first_record)) &&
...
static void serverObservationPrevPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    discard                     l;
    server_lifecycle_context_t *context = serverContext(t);
    ...
    {
        requireServer(bytes[i] == context->expected_destination_payload[context->cover_downstream_bytes %
                                                                        context->expected_destination_payload_len],
                      "downstream observation changed or reordered cover bytes");
        ++context->cover_downstream_bytes;
    }
    bufferpoolReuseBuffer(context->pool, buf);
}

static void testServerObservationDoesNotRetainBuffers(void)
{
    const uint8_t  record[] = {0x16, 0x03, 0x03, 0x00, 0x05, 0x0b, 0x00, 0x00, 0x01, 0xa5};
    const uint32_t chunks[] = {sizeof(record), 1};
    for (size_t split = 0; split < sizeof(chunks) / sizeof(chunks[0]); ++split)
    {
        server_lifecycle_fixture_t fixture;
        realityserver_lstate_t    *ls                    = serverFixturePrepareVisitorPending(&fixture);
        fixture.prev->fnPayloadD                         = serverObservationPrevPayload;
        fixture.context.expected_destination_payload     = record;
        fixture.context.expected_destination_payload_len = sizeof(record);

        /* Warm the input/forwarding slots, then detect retention on every callback. */
        ...
        bufferpoolCachedTierCountsForTest(fixture.pool, &large_before, &small_before, NULL, NULL);
        for (uint32_t repetition = 0; repetition < 4096; ++repetition)
        {
            for (uint32_t offset = 0; offset < sizeof(record); offset += chunks[split])
            {
                ...
                bufferpoolCachedTierCountsForTest(fixture.pool, &large_after, &small_after, NULL, NULL);
                ...
                requireServer(ls->mode == kRealityServerModePending && ! ls->session_keys_ready &&
                                  ! ls->tls_capture.server_ready && ! ls->protected_init_sent,
                              "non-ServerHello cover traffic changed pending state");
            }
        }
        requireServer(fixture.context.cover_downstream_bytes == 4096 * sizeof(record),
                      "pending destination bytes were not forwarded exactly once");
        realityserverLinestateDestroy(ls);
        serverFixtureDestroy(&fixture);
    }
}

static void testServerObservationTls12Flight(void)
{
    reality_tls_handshake_fixture_t *handshake = memoryAllocate(sizeof(*handshake));
    requireServer(handshake != NULL && realityTestBuildTlsHandshakeFixtureForCipher(
                                           kRealityV2Tls12, "ECDHE-RSA-AES128-GCM-SHA256", handshake),
                  "failed to build incremental TLS 1.2 observation fixture");
    uint32_t client_hello_len = serverTlsRecordLength(
        handshake->client_flight, handshake->client_flight_len, "invalid ClientHello fixture record");
    for (uint32_t fragmented = 0; fragmented < 2; ++fragmented)
    {
        server_lifecycle_fixture_t fixture;
        realityserver_lstate_t    *ls = serverFixturePrepareVisitorPending(&fixture);
        requireServer(
            realityserverProcessUpstream(fixture.reality,
                                         fixture.line,
                                         serverBufferFromBytes(&fixture, handshake->client_flight, client_hello_len)) &&
                ls->tls_capture.client_ready && ! ls->session_keys_ready,
            "ClientHello did not prepare pending observation");
        size_t chunk = fragmented ? 1 : handshake->server_flight_len;
        for (size_t offset = 0; offset < handshake->server_flight_len; offset += chunk)
        {
            requireServer(realityserverObserveDownstreamHandshake(
                              fixture.reality, fixture.line, handshake->server_flight + offset, chunk),
                          "incremental TLS 1.2 observation failed");
        }
        requireServer(ls->mode == kRealityServerModePending && ls->session_keys_ready &&
                          ls->record_profile.profile_id == kRealityV2RecordProfileTls12Gcm &&
                          ls->server_record_tracker.protected_epoch &&
                          ls->server_record_tracker.next_sequence == handshake->accessor_binding.next_read_sequence &&
                          ls->server_record_tracker.explicit_nonce_sample_count ==
                              handshake->accessor_binding.next_read_sequence &&
                          ls->server_record_tracker.sequence_pattern && ! ls->server_record_tracker.failed &&
                          ! ls->protected_init_sent && fixture.context.destination_upstream_records == 1,
                      "single-pass observation changed TLS 1.2 keys, sequence, or nonce state");
        reality_v2_session_material_t expected;
        realityserver_tstate_t       *ts = tunnelGetState(fixture.reality);
        requireServer(realityV2DeriveSessionMaterial(ts->root_key, &ls->tls_capture.binding, &expected) &&
                          memoryEqual(ls->session_id, expected.session_id, sizeof(ls->session_id)) &&
                          memoryEqual(ls->c2s_key, expected.c2s_key, sizeof(ls->c2s_key)) &&
                          memoryEqual(ls->s2c_key, expected.s2c_key, sizeof(ls->s2c_key)),
                      "incremental observation derived incorrect session material");
        memoryZero(&expected, sizeof(expected));
        realityserverLinestateDestroy(ls);
        serverFixtureDestroy(&fixture);
    }

    /* Complete ServerHello before its containing record ends, both with and
     * without an already captured ClientHello. The suffix is another handshake. */
    const uint8_t  suffix[]  = {0x0b, 0x00, 0x00, 0x01, 0xa5};
    const uint8_t *server    = handshake->server_flight;
    size_t         hello_len = 4 + ((size_t) server[6] << 16) + ((size_t) server[7] << 8) + server[8];
    uint8_t        record[512];
    requireServer(server[0] == 0x16 && server[5] == 0x02 && hello_len + 5 + sizeof(suffix) <= sizeof(record) &&
                      hello_len + 5 <= handshake->server_flight_len,
                  "ServerHello fixture does not fit the partial-record test");
    memoryCopy(record, server, hello_len + 5);
    size_t body_len = hello_len + sizeof(suffix);
    record[3]       = (uint8_t) (body_len >> 8);
    record[4]       = (uint8_t) body_len;
    memoryCopy(record + 5 + hello_len, suffix, sizeof(suffix));
    for (uint32_t early_server = 0; early_server < 2; ++early_server)
    {
        server_lifecycle_fixture_t fixture;
        realityserver_lstate_t    *ls = serverFixturePrepareVisitorPending(&fixture);
        if (! early_server)
        {
            requireServer(realityserverProcessUpstream(
                              ...
                              fixture.line,
                              serverBufferFromBytes(&fixture, handshake->client_flight, client_hello_len)),
                          "partial-record ClientHello failed");
        }
        for (size_t offset = 0; offset < hello_len + 5; ++offset)
        {
            requireServer(realityserverObserveDownstreamHandshake(fixture.reality, fixture.line, record + offset, 1),
                          "partial-record ServerHello observation failed");
        }
        requireServer(ls->tls_capture.server_ready && ls->session_keys_ready == ! early_server &&
                          ls->server_record_tracker.record_remaining == sizeof(suffix),
                      "ServerHello completion did not preserve the unfinished record");
        if (early_server)
        {
            requireServer(realityserverProcessUpstream(
                              fixture.reality,
                              fixture.line,
                              serverBufferFromBytes(&fixture, handshake->client_flight, client_hello_len)),
                          "early ServerHello was not resolved by ClientHello");
        }
        requireServer(ls->session_keys_ready && ls->mode == kRealityServerModePending &&
                          realityserverObserveDownstreamHandshake(
                              fixture.reality, fixture.line, record + 5 + hello_len, sizeof(suffix)) &&
                          ls->server_record_tracker.record_remaining == 0 &&
                          ls->server_record_tracker.record_header_length == 0 && ! ls->server_record_tracker.failed,
                      "profile installation mid-record lost the remaining bytes");
        realityserverLinestateDestroy(ls);
        serverFixtureDestroy(&fixture);
    }
...
static void testServerObservationEarlyServerHello(void)
{
    const uint16_t versions[] = {kRealityV2Tls12, kRealityV2Tls13};
    for (size_t version = 0; version < sizeof(versions) / sizeof(versions[0]); ++version)
    {
        reality_tls_handshake_fixture_t *handshake = memoryAllocate(sizeof(*handshake));
        requireServer(handshake != NULL && realityTestBuildTlsHandshakeFixture(versions[version], handshake),
                      "failed to build early ServerHello fixture");
        uint32_t client_hello_len = serverTlsRecordLength(
            handshake->client_flight, handshake->client_flight_len, "invalid early ClientHello record");
        uint32_t server_hello_len = serverTlsRecordLength(
            handshake->server_flight, handshake->server_flight_len, "invalid early ServerHello record");
        /* Boundary-only ServerHello, premature protected flight, and re-entrant
         * death while flushing the failed TLS 1.2 flight's pending client bytes. */
        for (uint32_t scenario = 0; scenario < 3; ++scenario)
        {
            server_lifecycle_fixture_t fixture;
            realityserver_lstate_t    *ls           = serverFixturePrepareVisitorPending(&fixture);
            size_t                     observed_len = scenario == 0 ? server_hello_len : handshake->server_flight_len;
            for (size_t offset = 0; offset < observed_len; ++offset)
            {
                requireServer(realityserverObserveDownstreamHandshake(
                                  fixture.reality, fixture.line, handshake->server_flight + offset, 1),
                              "early observation must defer TLS 1.2 tracker failure");
            }
            requireServer(ls->mode == kRealityServerModePending && ls->tls_capture.server_ready &&
                              ! ls->tls_capture.client_ready && ! ls->session_keys_ready &&
                              ls->server_record_tracker.failed == (scenario != 0),
                          "early ServerHello did not preserve deferred tracker state");
            bool fallback                                    = versions[version] == kRealityV2Tls12 && scenario != 0;
            bool kill_on_flush                               = fallback && scenario == 2;
            fixture.context.finish_on_destination_payload    = kill_on_flush;
            fixture.context.kill_on_prev_finish              = kill_on_flush;
            fixture.context.expected_destination_payload     = handshake->client_flight;
            fixture.context.expected_destination_payload_len = client_hello_len;
            requireServer(realityserverProcessUpstream(fixture.reality,
                                                       fixture.line,
                                                       serverBufferFromBytes(&fixture, handshake->client_flight, 2)) &&
                              fixture.context.matched_destination_payloads == 0,
                          "partial ClientHello was flushed prematurely");
            bool alive = realityserverProcessUpstream(
                fixture.reality,
                fixture.line,
                serverBufferFromBytes(&fixture, handshake->client_flight + 2, client_hello_len - 2));
            requireServer(alive == ! kill_on_flush && fixture.context.matched_destination_payloads == 1 &&
                              fixture.context.destination_upstream_records == 1,
                          "early ServerHello resolution did not settle client bytes exactly once");
            if (kill_on_flush)
            {
                requireServer(! lineIsAlive(fixture.line) && strcmp(fixture.context.events, "TD") == 0,
                              "Visitor fallback did not propagate re-entrant line death");
            }
            else
            {
                requireServer(ls->mode == (fallback ? kRealityServerModeVisitor : kRealityServerModePending) &&
                                  ls->session_keys_ready == ! fallback && ! ls->protected_init_sent &&
                                  bufferstreamIsEmpty(&ls->read_stream),
                              "early ServerHello selected the wrong derivation or Visitor policy");
                realityserverLinestateDestroy(ls);
            }
            serverFixtureDestroy(&fixture);
        }
...
{
    reality_tls_handshake_fixture_t *handshake = memoryAllocate(sizeof(*handshake));
    requireServer(handshake != NULL && realityTestBuildTlsHandshakeFixture(kRealityV2Tls13, handshake),
    ...
    uint32_t client_hello_len = serverTlsRecordLength(handshake->client_flight,
                                                      handshake->client_flight_len,
                                                      "real TLS 1.3 client flight has an invalid ClientHello record");
    requireServer(
        handshake->client_flight[0] == kRealityServerTlsHandshake &&
            realityserverProcessUpstream(fixture.reality,
                                         fixture.line,
                                         serverBufferFromBytes(&fixture, handshake->client_flight, client_hello_len)) &&
            ls->tls_capture.client_ready && ! ls->session_keys_ready &&
    ...
    requireServer(realityserverObserveDownstreamHandshake(
                      fixture.reality, fixture.line, handshake->server_flight, handshake->server_flight_len) &&
                      ls->session_keys_ready && ls->tls_capture.binding.tls_version == kRealityV2Tls13 &&
                      ls->tls13_pre_request_cover_records_remaining ==
    ...
    while (offset < handshake->client_flight_len)
    {
        ...
        requireServer(realityserverProcessUpstream(
                          fixture.reality, fixture.line, serverBufferFromBytes(&fixture, record, record_len)),
                      "server rejected a real TLS 1.3 client handshake record");

        if (record[0] == kRealityServerTlsApplicationData)
        {
            requireServer(ls->mode == kRealityServerModePending && ls->tls13_pre_request_cover_records_remaining == 0 &&
    ...
    requireServer(
        realityserverProcessUpstream(fixture.reality,
                                     ...
                                     buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0)) &&
            ls->mode == kRealityServerModeHandoffAwaitConfirm && ls->handoff_request_authenticated &&
            ls->handoff_ack_sent && ls->tls13_pre_request_cover_records_remaining == 0 && ls->c2s_recv_seq == 1 &&
            ls->s2c_send_seq == 1,
...
static void testServerTls13CandidateAllowanceIsSingleUse(void)
{
    ...
    requireServer(realityserverProcessUpstream(fixture.reality, fixture.line, first) &&
                      ls->mode == kRealityServerModePending && ls->tls13_pre_request_cover_records_remaining == 0 &&
                      ls->sniffing_attempts == 0 && ls->c2s_recv_seq == 0,
    ...
    requireServer(realityserverProcessUpstream(fixture.reality, fixture.line, second) &&
                      ls->mode == kRealityServerModeVisitor && ls->tls13_pre_request_cover_records_remaining == 0 &&
                      ls->sniffing_attempts == 1 && ls->c2s_recv_seq == 0 &&
                      fixture.context.destination_upstream_records == 2 &&
                      fixture.context.matched_destination_payloads == 2 && strcmp(fixture.context.events, "TT") == 0,
...
static void testServerTls13NonCandidateDoesNotConsumeAllowance(void)
{
    ...
    requireServer(
        realityserverProcessUpstream(
            fixture.reality, fixture.line, serverBufferFromBytes(&fixture, ccs, sizeof(ccs))) &&
            ls->mode == kRealityServerModePending &&
            ls->tls13_pre_request_cover_records_remaining == kRealityServerTls13PreRequestCoverRecordAllowance &&
            ls->sniffing_attempts == 0 && ls->c2s_recv_seq == 0 && fixture.context.destination_upstream_records == 1 &&
            fixture.context.matched_destination_payloads == 1,
...
static void testServerTls13NonRequestControlsStayPending(void)
{
    ...
    for (size_t i = 0; i < sizeof(control_kinds) / sizeof(control_kinds[0]); ++i)
    {
        ...
        requireServer(expected_len <= sizeof(expected), "server non-REQUEST control fixture overflow");
        ...
        requireServer(realityserverProcessUpstream(fixture.reality, fixture.line, control) &&
                          ls->mode == kRealityServerModePending && ! ls->handoff_request_authenticated &&
                          ! ls->handoff_ack_sent && ! ls->protected_init_sent && ls->c2s_recv_seq == 0 &&
                          ls->s2c_send_seq == 0 && ls->sniffing_attempts == 0 &&
                          ls->tls13_pre_request_cover_records_remaining == 0 &&
                          fixture.context.destination_upstream_records == 1 &&
                          fixture.context.matched_destination_payloads == 1 && strcmp(fixture.context.events, "T") == 0,
...
static void testServerTls13FailedRequestIsByteExactFallback(void)
{
    ...
    for (uint8_t allowance = 0; allowance <= kRealityServerTls13PreRequestCoverRecordAllowance; ++allowance)
    {
        server_lifecycle_fixture_t fixture;
        serverFixtureInitialize(&fixture);
        serverFixtureAddDestination(&fixture);
        ...
        sbuf_t  *request      = buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0);
        ...
        requireServer(realityserverProcessUpstream(fixture.reality, fixture.line, request) &&
                          ls->mode == kRealityServerModePending && ! ls->handoff_request_authenticated &&
                          ! ls->handoff_ack_sent && ! ls->protected_init_sent && ls->c2s_recv_seq == 0 &&
                          ls->s2c_send_seq == 0 && ls->tls13_pre_request_cover_records_remaining == 0 &&
...
static void testServerTls12CandidateBudgetIsUnchanged(void)
{
    ...
    sbuf_t       *candidate   = buildServerInboundRecord(
        &fixture, kRealityV2Tls12, &profile, kRealityV2RecordKindApplicationData, plaintext, sizeof(plaintext), 0);
...
static void testServerTls13DuplicateConfirmDoesNotReinitializeNext(void)
{
    ...
    requireServer(
        realityserverProcessUpstream(fixture.reality,
                                     fixture.line,
                                     buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0)) &&
            ls->mode == kRealityServerModeHandoffAwaitConfirm && strcmp(fixture.context.events, "K") == 0,
        "server did not prepare duplicate-CONFIRM fixture");

    sbuf_t *confirm   = buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffConfirm, 1);
    sbuf_t *duplicate = buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffConfirm, 1);
    requireServer(realityserverProcessUpstream(fixture.reality, fixture.line, confirm) &&
                      ls->mode == kRealityServerModeAuthorized && ls->protected_init_sent &&
    ...
    requireServer(strchr(fixture.context.events, 'I') == strrchr(fixture.context.events, 'I'),
...
static void testServerTls13PendingForwardReentrantFinishKillsLine(void)
{
    ...
    requireServer(
        ! realityserverProcessUpstream(
            fixture.reality, fixture.line, buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffAck, 0)) &&
            ! lineIsAlive(fixture.line) && strcmp(fixture.context.events, "TD") == 0,
...
static void testServerTls13PendingBudgetCannotOverwriteReentrantHandoff(void)
{
    ...
    ts->sniffing_attempts                                             = 1;
    fixture.context.require_allowance_consumed_on_destination_payload = true;
    fixture.context.request_on_destination_payload =
        buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0);
    ...
    requireServer(realityserverProcessUpstream(fixture.reality, fixture.line, record) &&
                      ...
                      ls->mode == kRealityServerModeHandoffAwaitConfirm && ls->handoff_request_authenticated &&
                      ls->handoff_ack_sent && ls->destination_downstream_cutoff && ls->sniffing_attempts == 0 &&
                      ls->tls13_pre_request_cover_records_remaining == 0 && ls->c2s_recv_seq == 1 &&
                      ls->s2c_send_seq == 1 && fixture.context.destination_upstream_records == 1 &&
                      fixture.context.handoff_ack_records == 1 && strcmp(fixture.context.events, "TK") == 0,
...
static void testServerTls13LineDeathAtHandoffCallbacks(void)
{
    ...
    const uint32_t first_part_len = 8;
    realityserverTunnelDownStreamPayload(
        fixture.reality, fixture.line, serverBufferFromBytes(&fixture, cover_record, first_part_len));
    requireServer(
        realityserverProcessUpstream(fixture.reality,
                                     fixture.line,
                                     buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0)) &&
            ls->mode == kRealityServerModeHandoffAwaitBoundary && strcmp(fixture.context.events, "c") == 0,
        "server did not prepare the final-cover-prefix death fixture");

    uint8_t final_callback[(sizeof(cover_record) - first_part_len) + sizeof(suppressed_record)];
    memoryCopy(final_callback, cover_record + first_part_len, sizeof(cover_record) - first_part_len);
    memoryCopy(final_callback + sizeof(cover_record) - first_part_len, suppressed_record, sizeof(suppressed_record));
    fixture.context.kill_on_cover_payload = true;
    realityserverTunnelDownStreamPayload(
        fixture.reality, fixture.line, serverBufferFromBytes(&fixture, final_callback, sizeof(final_callback)));
    requireServer(! lineIsAlive(fixture.line) && fixture.context.cover_downstream_bytes == sizeof(cover_record) &&
                      memoryEqual(fixture.context.cover_downstream, cover_record, sizeof(cover_record)) &&
                      fixture.context.handoff_ack_records == 0 && strcmp(fixture.context.events, "ccF") == 0,
    ...
    requireServer(
        ! realityserverProcessUpstream(fixture.reality,
                                       fixture.line,
                                       buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0)) &&
            ! lineIsAlive(fixture.line) && fixture.context.handoff_ack_records == 1 &&
    ...
    requireServer(
        ! realityserverProcessUpstream(fixture.reality,
                                       fixture.line,
                                       buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffConfirm, 1)) &&
            ! lineIsAlive(fixture.line) && strcmp(fixture.context.events, "KF") == 0,
    ...
    requireServer(
        ! realityserverProcessUpstream(fixture.reality,
                                       fixture.line,
                                       buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffConfirm, 1)) &&
            ! lineIsAlive(fixture.line) && strcmp(fixture.context.events, "KFIU") == 0,
    ...
    requireServer(! realityserverProcessUpstream(fixture.reality,
                                                 ...
                                                 buildServerInboundRecord(&fixture,
                                                                          ...
                                                                          sizeof(protected_application),
                                                                          2)) &&
                      ! lineIsAlive(fixture.line) && strcmp(fixture.context.events, "KFIAU") == 0,
...
static void testServerTls13DestinationCallbacksStaySuppressed(void)
{
    ...
    requireServer(
        realityserverProcessUpstream(
            fixture.reality, fixture.line, buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffRequest, 0)),
        "server rejected REQUEST during cutoff callback injection");
    realityserver_lstate_t *ls = lineGetState(fixture.line, fixture.reality);
    requireServer(ls->mode == kRealityServerModeHandoffAwaitConfirm && ls->destination_downstream_cutoff &&
                      ls->destination_up_finished && ! ls->protected_init_sent && ! ls->prev_est_sent &&
                      fixture.context.destination_callbacks_injected == 5 &&
                      fixture.context.cover_downstream_bytes == 0 && strcmp(fixture.context.events, "K") == 0,
    ...
    requireServer(
        realityserverProcessUpstream(fixture.reality,
                                     ...
                                     buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffConfirm, 1)) &&
            ls->mode == kRealityServerModeAuthorized && ls->protected_init_sent &&
            strcmp(fixture.context.events, "KI") == 0,
    ...
    requireServer(
        realityserverProcessUpstream(
            fixture.reality, fixture.line, buildServerInboundControl(&fixture, kRealityV2RecordKindHandoffConfirm, 1)),
        "server rejected CONFIRM during destination-close callback injection");
    ls = lineGetState(fixture.line, fixture.reality);
    requireServer(ls->mode == kRealityServerModeAuthorized && ls->destination_up_finished && ls->protected_init_sent &&
                      ! ls->closing_destination_for_authorized && ! ls->prev_est_sent &&
                      fixture.context.destination_callbacks_injected == 5 &&
                      fixture.context.cover_downstream_bytes == 0 && strcmp(fixture.context.events, "KFI") == 0,
...
static void testServerTls13HandoffTailBound(void)
{
    ...
    for (uint32_t i = 0; i < kRealityServerMaxHandoffTailRecords; ++i)
    {
        requireServer(
            realityserverProcessUpstream(fixture.reality,
                                         fixture.line,
                                         serverBufferFromBytes(&fixture, late_cover_record, sizeof(late_cover_record))),
            "server closed before reaching the handoff-tail record bound");
    }
    requireServer(ls->c2s_recv_seq == 1 &&
                      fixture.context.destination_upstream_records == kRealityServerMaxHandoffTailRecords &&
...
void realityTestServerCloseLifecycle(void)
{
    ...
    testServerObservationDoesNotRetainBuffers();
    testServerObservationTls12Flight();
    testServerObservationEarlyServerHello();
...
static void serverSizingCapture(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    requireServer(record_len >= kRealityV2TlsRecordHeaderSize, "server sizing capture received a truncated TLS record");

    reality_v2_record_descriptor_t descriptor;
    requireServer(
        realityV2BuildRecordDescriptor(
            ls->tls_capture.binding.tls_version, &ls->record_profile, kRealityV2RecordKindApplicationData, &descriptor),
        "server sizing descriptor construction failed");
    uint32_t chunk_len =
        min(context->expected_payload_len - context->recovered_payload_len, (uint32_t) kRealityV2MaxPlaintextFragment);
    reality_v2_record_layout_t layout;
    requireServer(realityV2CalculateDescriptorLayout(&descriptor, chunk_len, &layout) &&
                      ...
                      record[0] == descriptor.outer_content_type && record[1] == 0x03 && record[2] == 0x03 &&
                                ...
                                bool kill_after_first_record)
{
    ...
    requireServer(input_len <= sbufGetMaximumWriteableSize(input), "server sizing input does not fit the test buffer");
    ...
    *(server_sizing_context_t **) tunnelGetState(capture) = &context;
    ...
    uint32_t expected_records = (input_len + kRealityV2MaxPlaintextFragment - 1U) / kRealityV2MaxPlaintextFragment;
    ...
    {
        requireServer(sent && context.record_count == expected_records && context.recovered_payload_len == input_len,
                      "server sizing sender emitted the wrong chunk sequence");
        if (input_len == 32768)
        {
            requireServer(context.record_count == 2, "server 32768-byte callback emitted a tiny third record");
        }
    }
...
void realityTestServerRecordSizing(void)
{
    static const struct
    {
        ...
        {kRealityV2Tls12, {kRealityV2RecordProfileTls12Gcm, kRealityV2Tls12GcmPrefixSize, 0, 0}},
        {kRealityV2Tls12, {kRealityV2RecordProfileTls12Cbc, kRealityV2Tls12CbcPrefixSize, 16, 20}},
    };
    ...
    for (size_t i = 0; i < sizeof(profiles) / sizeof(profiles[0]); ++i)
    {
        ...
        {
            runServerSizingCase(profiles[i].tls_version, &profiles[i].profile, input_lengths[j], false);
        }
