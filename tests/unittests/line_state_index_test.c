...
static void testDenseSlots(void)
{
    ...
    for (unsigned int iteration = 0; iteration < 3; ++iteration)
    {
        ...
        lineRef(line);
        require(atomicLoadU32Relaxed(&line->refc) == 2, "lineRef did not retain the line");
        lineUnref(line);
        require(atomicLoadU32Relaxed(&line->refc) == 1 && lineIsAlive(line),
                "lineUnref released the creator reference");
...
{
#ifdef OS_LINUX
    // Invalid physical counts must be rejected in Release as well as Debug.
    const uint32_t invalid_counts[] = {0, UINT32_MAX};
    for (size_t i = 0; i < ARRAY_SIZE(invalid_counts); ++i)
    {
        pid_t child = fork();
        require(child >= 0, "failed to fork invalid-reference test");
        if (child == 0)
        {
            line_t line = {.alive = true, .refc = invalid_counts[i]};
            lineRef(&line);
            _Exit(0);
        }
        int status;
        require(waitpid(child, &status, 0) == child && WIFEXITED(status) && WEXITSTATUS(status) == 1,
                "lineRef accepted an invalid reference count");
    }
#endif
    testLargeOffsets();
    testDenseSlots();
