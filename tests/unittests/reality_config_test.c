...
int main(void)
{
    ...
    _Static_assert(sizeof(max_salt) - 1 == kRealityV2MaxSaltByteLength,
                   "root-key test salt boundary drifted");
    _Static_assert(sizeof(long_password) - 1 == kRealityV2MaxPasswordByteLength + 1,
                   "root-key oversized password fixture drifted");
    _Static_assert(sizeof(long_salt) - 1 == kRealityV2MaxSaltByteLength + 1,
                   "root-key oversized salt fixture drifted");

    uint8_t root_key[kRealityV2KeySize];
    require(realityV2DeriveRootKey("p", "s", 1, root_key),
            "one-byte Reality credentials were rejected");
    ...
    require(! realityV2DeriveRootKey("", "s", 1, root_key),
            "empty Reality password was accepted");
    require(! realityV2DeriveRootKey("p", "", 1, root_key),
            "empty Reality salt was accepted");
    require(! realityV2DeriveRootKey(long_password, "s", 1, root_key),
            "oversized Reality password was accepted at one iteration");
    require(! realityV2DeriveRootKey("p", long_salt, 1, root_key),
            "oversized Reality salt was accepted");
    require(! realityV2DeriveRootKey("p", "s", 0, root_key),
            "zero Reality KDF iterations were accepted");
    ...
    require(! realityV2DeriveRootKey(NULL, "s", 1, root_key) &&
                ! realityV2DeriveRootKey("p", NULL, 1, root_key) &&
    ...
    requireEqual(root_key, expected_root_key, sizeof(expected_root_key),
