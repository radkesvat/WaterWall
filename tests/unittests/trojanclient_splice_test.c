...
static void performAction(unsigned boundary)
{
    ...
    if (action == 7)
    {
        ...
        twfRequire(lineIsAlive(f.application), "TrojanClient destroyed a borrowed line");
...
static void begin(bool udp, const char *target, uint32_t pool_size, unsigned init_action, bool context)
{
    ...
    f.udp                                           = udp;
    twfWorkerEnvSetupWithBufferSizes(&f.env, pool_size, 512, 352, 8192, pool_size);
    if (test_wid != 0)
    {
    ...
    twfRequire(f.node.flags == kNodeFlagSupportsSplice && f.node.required_padding_left == 331 &&
...
static void testUdp(unsigned form, bool pipe)
{
    ...
    for (unsigned i = 0; i < 4; ++i)
    {
        for (unsigned pad = 0; pad < 2; ++pad)
        {
            ...
            if (n > 8192)
                twfRequire(f.deliveries_up == 0 && lineIsAlive(f.application),
                           "oversized local datagram killed association");
            else
            {
...
static void testMalformed(void)
{
    ...
    for (unsigned fault = 0; fault < 5; ++fault)
    {
        ...
        twfRequire(! lineIsAlive(f.application) && ! lineIsAlive(f.carrier) && f.deliveries_down == 0,
...
static void testPrefixesAndFragments(void)
{
    ...
    twfRequire(lineIsAlive(f.application) && f.deliveries_down == 3, "valid fragment count rejected");
    f.paused_down = false;
    f.t->fnResumeU(f.t, f.application);
...
static void testFirstPayload(void)
{
    ...
    for (unsigned udp = 0; udp < 2; ++udp)
        for (unsigned representation = 0; representation < 3; ++representation)
        {
            ...
            uint32_t expected_padding = ordinary ? sbufGetLeftCapacity(body) - 68 - (udp ? 11 : 0)
                                                 : bufferpoolGetLargeBufferPadding(f.env.pool);
...
static void testFirstPayloadReuse(void)
{
    ...
    const uint32_t tcp_headers[] = {68, 80, 320};
    const uint32_t udp_headers[] = {11, 23, 263};
    for (unsigned udp = 0; udp < 2; ++udp)
        for (unsigned form = 0; form < 3; ++form)
        {
            uint32_t headers = udp ? 68 + udp_headers[form] : tcp_headers[form];
