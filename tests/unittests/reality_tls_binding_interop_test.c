...
static const uint8_t *requireLastRecord(const uint8_t *flight, size_t flight_len, uint16_t *last_body_len,
                                        const char *message)
{
    size_t         offset      = 0;
...
static void requireSingleProtectedRecordBody(const uint8_t *flight, size_t flight_len, uint16_t expected_body_len,
                                             const char *message)
{
    require(flight_len == (size_t) expected_body_len + kRealityV2TlsRecordHeaderSize && flight[0] == 0x17 &&
                flight[1] == 0x03 && flight[2] == 0x03 &&
                (((uint16_t) flight[3] << 8) | flight[4]) == expected_body_len,
            message);
}

static const uint8_t *requireCloseFlightShape(const uint8_t *flight, size_t flight_len, uint16_t version,
                                              uint8_t profile, const char *message)
{
    ...
    require(last_record[0] == expected_type && last_record[1] == 0x03 && last_record[2] == 0x03 &&
...
static void requireParserMatchesAccessor(uint16_t version, const char *cipher, uint8_t expected_profile, bool resumed)
{
    ...
    bool fixture_ok = resumed ? realityTestBuildResumedTls12HandshakeFixtureForCipher(cipher, fixture)
                              : realityTestBuildTlsHandshakeFixtureForCipher(version, cipher, fixture);
    require(fixture_ok, "real TLS handshake fixture failed");
    require(fixture->session_reused == resumed, "real TLS handshake fixture reported the wrong resumption state");
    ...
    require(realityserverTlsParserFeed(&client_parser, fixture->client_flight, fixture->client_flight_len, &capture),
            "passive ClientHello parsing failed for real handshake");
    require(realityserverTlsParserFeed(&server_parser, fixture->server_flight, fixture->server_flight_len, &capture),
    ...
    if (version == kRealityV2Tls12)
    {
        ...
        require(fixture->accessor_binding.tls12_sequences_valid, "TlsClient accessor did not expose TLS 1.2 sequences");
        require(realityV2SelectRecordProfile(capture.binding.tls_version, capture.binding.cipher_suite, &profile),
        ...
        require(
            realityserverTls12RecordTrackerFeed(&client_tracker, fixture->client_flight, fixture->client_flight_len) &&
        ...
        else if (profile.profile_id == kRealityV2RecordProfileTls12Cbc)
        {
            ...
            const uint8_t *client_last_record   = requireLastRecord(fixture->client_flight,
                                                                  fixture->client_flight_len,
                                                                  &client_last_body_len,
                                                                  "failed to locate the last client handshake record");
            const uint8_t *server_last_record   = requireLastRecord(fixture->server_flight,
    ...
    else
    {
        ...
        require(protected_handshake[0] == 0x17 && protected_handshake_body_len == kRealityV2ControlMaxTlsRecordBody,
...
int main(void)
{
    requireParserMatchesAccessor(0x0303, "ECDHE-RSA-AES128-GCM-SHA256", kRealityV2RecordProfileTls12Gcm, false);
    requireParserMatchesAccessor(0x0303, "ECDHE-RSA-AES256-GCM-SHA384", kRealityV2RecordProfileTls12Gcm, false);
    ...
    requireParserMatchesAccessor(0x0303, "ECDHE-RSA-CHACHA20-POLY1305", kRealityV2RecordProfileTls12ChaCha, false);
    requireParserMatchesAccessor(0x0304, NULL, kRealityV2RecordProfileTls13Aead, false);
    requireParserMatchesAccessor(0x0303, "ECDHE-RSA-AES128-GCM-SHA256", kRealityV2RecordProfileTls12Gcm, true);
    requireParserMatchesAccessor(0x0303, "ECDHE-RSA-AES128-SHA", kRealityV2RecordProfileTls12Cbc, true);
    requireParserMatchesAccessor(0x0303, "ECDHE-RSA-CHACHA20-POLY1305", kRealityV2RecordProfileTls12ChaCha, true);
    return 0;
}
