...
static void clientPrevPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    requireClient(sbufGetLength(buf) == 1 && payload[0] == 0x5a, "client delivered the wrong application payload");
...
static void clientTlsPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    if (context->tls_peer_rbio != NULL)
    {
        ...
        requireClient(WW_BSSL_BIO_write(context->tls_peer_rbio, sbufGetRawPtr(buf), len) == len,
...
static wcrypto_status_t clientLifecycleDecrypt(void *context, unsigned char *dst, size_t dst_capacity,
...
static void clientNextHandoffPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    {
        requireClient(realityV2ParseControl(expected_kind, plaintext + payload_offset, payload_len),
                      "client handoff lifecycle emitted invalid control semantics");
        clientEvent(context, expected_kind == kRealityV2RecordKindHandoffRequest ? 'R' : 'C');
        if (expected_kind == kRealityV2RecordKindHandoffConfirm && context->advance_kind_after_confirm)
        {
...
static void clientNextPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    reality_v2_record_descriptor_t descriptor;
    requireClient(
        realityV2BuildRecordDescriptor(ls->tls_version, &ls->record_profile, kRealityV2RecordKindAlert, &descriptor),
    ...
    requireClient(realityV2BuildRecordAad(&descriptor,
                      ...
                      wCryptoChaCha20Poly1305Decrypt(
                          inner, sizeof(inner), ciphertext, ciphertext_len, aad, aad_len, nonce, ls->c2s_key) ==
                          kWCryptoOk,
                  "failed to decrypt client final alert");
    uint32_t payload_offset = 0;
    uint32_t payload_len    = 0;
    requireClient(realityV2ValidateInnerPlaintext(
...
static void clientFixtureInitialize(client_lifecycle_fixture_t *fixture)
{
    ...
    bufferpoolUpdateAllocationPaddings(
        fixture->pool, kRealityClientMaxFramePrefixSize, kRealityClientMaxFramePrefixSize);
...
static void clientFixtureSetFailureMode(client_lifecycle_fixture_t *fixture, bool kill_on_payload,
                                        bool kill_on_next_finish)
{
...
static sbuf_t *clientBufferFromBytes(client_lifecycle_fixture_t *fixture, const uint8_t *data, uint32_t len)
{
...
static sbuf_t *buildClientInboundRecord(client_lifecycle_fixture_t *fixture, uint16_t tls_version,
...
static void clientTlsCredentialPath(char *out, size_t out_size, const char *name)
{
    ...
    int    n        = snprintf(out, out_size, "%.*s/tests/cases/tls_roundtrip/%s", (int) root_len, __FILE__, name);
    requireClient(n > 0 && (size_t) n < out_size, "client TLS fixture credential path overflow");
}
...
static uint32_t clientTransferTlsBytes(void *from, void *to)
{
    ...
    while (true)
    {
        ...
        requireClient(WW_BSSL_BIO_write(to, bytes, n) == n, "client TLS fixture failed to transfer handshake bytes");
        total += (uint32_t) n;
    }
}
...
static void clientFixtureEnableTls13TakeoverWithTickets(client_lifecycle_fixture_t *fixture, size_t ticket_count)
{
    ...
    if (ticket_count > 1)
    {
        requireClient(WW_BSSL_SSL_set_max_send_fragment(fixture->tls_server_ssl, 512) == 1,
                      "client TLS fixture failed to separate multiple ticket records");
    }
    WW_BSSL_SSL_set_bio(fixture->tls_server_ssl, fixture->tls_server_rbio, fixture->tls_server_wbio);
...
static sbuf_t *clientDrainTlsServerWire(client_lifecycle_fixture_t *fixture, const char *failure_message)
{
    uint8_t wire[8192];
    int     wire_len = WW_BSSL_BIO_read(fixture->tls_server_wbio, wire, (int) sizeof(wire));
    requireClient(wire_len > 0 && WW_BSSL_BIO_read(fixture->tls_server_wbio, wire, (int) sizeof(wire)) <= 0,
                  failure_message);
...
static uint32_t clientCountTlsRecords(const sbuf_t *flight)
{
    ...
    while (offset < len)
    {
        requireClient(len - offset >= kClientTlsRecordHeaderSize, "client TLS flight ended in a partial header");
        uint32_t body_len   = ((uint32_t) bytes[offset + 3] << 8U) | (uint32_t) bytes[offset + 4];
...
static sbuf_t *clientCopyTlsRecord(client_lifecycle_fixture_t *fixture, const sbuf_t *flight, uint32_t record_index)
{
    const uint8_t *bytes  = sbufGetRawPtr(flight);
    uint32_t       len    = sbufGetLength(flight);
    uint32_t       offset = 0;
    ...
    {
        requireClient(len - offset >= kClientTlsRecordHeaderSize, "client TLS record copy found a partial header");
        uint32_t body_len   = ((uint32_t) bytes[offset + 3] << 8U) | (uint32_t) bytes[offset + 4];
        uint32_t record_len = kClientTlsRecordHeaderSize + body_len;
        requireClient(record_len <= len - offset, "client TLS record copy found a partial body");
...
static sbuf_t *clientBuildCoverApplicationRecord(client_lifecycle_fixture_t *fixture)
{
    ...
    requireClient(WW_BSSL_SSL_write(fixture->tls_server_ssl, plaintext, (int) sizeof(plaintext)) ==
...
{
    requireClient(WW_BSSL_SSL_key_update(fixture->tls_server_ssl, kClientKeyUpdateRequested) == 1 &&
                      WW_BSSL_SSL_write(fixture->tls_server_ssl, NULL, 0) == 0,
                  "client TLS fixture failed to emit requested KeyUpdate");
    sbuf_t *record = clientDrainTlsServerWire(fixture, "client TLS fixture emitted no KeyUpdate record");
    requireClient(clientCountTlsRecords(record) == 1, "client TLS fixture emitted an unexpected KeyUpdate flight");
    return record;
}
...
static sbuf_t *clientBuildHandoffAck(client_lifecycle_fixture_t *fixture)
{
    ...
    requireClient(
        realityV2SerializeControl(kRealityV2RecordKindHandoffAck, padding, sizeof(padding), control, sizeof(control)),
        "failed to serialize client ACK fixture");
    realityclient_lstate_t *ls  = lineGetState(fixture->line, fixture->reality);
    sbuf_t                 *ack = buildClientInboundRecord(
        fixture, kRealityV2Tls13, &ls->record_profile, kRealityV2RecordKindHandoffAck, control, sizeof(control), 0);
    memoryZero(padding, sizeof(padding));
    memoryZero(control, sizeof(control));
    return ack;
}

static sbuf_t *clientJoinRecords(client_lifecycle_fixture_t *fixture, sbuf_t *first, sbuf_t *second)
{
    ...
    memoryCopy(sbufGetMutablePtr(combined) + first_len, sbufGetRawPtr(second), second_len);
    lineReuseBuffer(fixture->line, first);
    lineReuseBuffer(fixture->line, second);
    return combined;
}

static void configureClientProfile(client_lifecycle_fixture_t *fixture, uint16_t tls_version,
...
static void runClientFragmentedAlert(uint16_t tls_version, const reality_v2_record_profile_t *profile)
{
    ...
    sbuf_t *whole = buildClientInboundRecord(
    ...
    for (uint32_t split = 1; split < whole_len; ++split)
    {
        ...
        reality_v2_record_descriptor_t descriptor;
        reality_v2_record_layout_t     layout;
        requireClient(realityV2BuildRecordDescriptor(tls_version, profile, kRealityV2RecordKindAlert, &descriptor) &&
        ...
        requireClient(realityclientProcessDownstream(
                          fixture.reality, fixture.line, clientBufferFromBytes(&fixture, record, split)),
                      "client rejected an incomplete alert fragment");
        requireClient(fixture.context.events_len == 0, "client emitted callbacks before the alert was complete");
        requireClient(
            ! realityclientProcessDownstream(
                fixture.reality, fixture.line, clientBufferFromBytes(&fixture, record + split, whole_len - split)),
...
static void testClientCoalescedRecords(uint16_t tls_version, const reality_v2_record_profile_t *profile,
                                       bool alert_first)
{
    client_lifecycle_fixture_t fixture;
    clientFixtureInitialize(&fixture);
    ...
    sbuf_t *first =
        buildClientInboundRecord(&fixture,
                                 tls_version,
                                 profile,
    ...
    lineReuseBuffer(fixture.line, second);
    requireClient(! realityclientProcessDownstream(
                      fixture.reality, fixture.line, clientBufferFromBytes(&fixture, combined, combined_len)),
...
static void testClientHandoffRequestAndQueue(void)
{
    ...
    requireClient(realityclientSendHandoffControl(fixture.reality, fixture.line, kRealityV2RecordKindHandoffRequest),
                  "client failed to send handoff request");
    realityclient_lstate_t *ls = lineGetState(fixture.line, fixture.reality);
    requireClient(strcmp(fixture.context.events, "R") == 0 && ls->handoff_request_sent && ls->c2s_send_seq == 1,
                  "client did not send exactly one sequence-zero request");
    requireClient(
        ! realityclientSendHandoffControl(fixture.reality, fixture.line, kRealityV2RecordKindHandoffRequest) &&
    ...
    requireClient(bufferqueueGetBufCount(&ls->pending_up) == 1 && strcmp(fixture.context.events, "R") == 0,
...
static void testClientNestedAckIsDeferredDuringCoverConsume(void)
{
    ...
    requireClient(realityclientProcessHandoffDownstream(fixture.reality, fixture.line, ack),
                  "client rejected a nested ACK while cover consume owned TLS state");
    ls = lineGetState(fixture.line, fixture.reality);
    requireClient(ls->phase == kRealityClientPhaseTls13AwaitAck && ls->handoff_cover_consume_in_progress &&
                      ! ls->handoff_ack_authenticated && ! ls->handoff_confirm_sent && ls->s2c_recv_seq == 0 &&
                      ls->c2s_send_seq == 1 && bufferstreamGetBufLen(&ls->handoff_stream) == ack_len &&
...
static void testClientFailedAckDelegatesOriginalCoverRecord(void)
{
    ...
    requireClient(cover_len <= sizeof(cover_snapshot), "client cover-record snapshot overflow");
    ...
    realityclient_lstate_t   *ls     = lineGetState(fixture.line, fixture.reality);
    client_tls_lstate_view_t *tls_ls = lineGetState(fixture.line, fixture.tls);
    requireClient(ls->phase == kRealityClientPhaseTls13AwaitAck && ! ls->handoff_ack_authenticated &&
                      ! ls->handoff_confirm_sent && ls->s2c_recv_seq == 0 && ls->c2s_send_seq == 1 &&
    ...
    requireClient(tls_ls->takeover_phase == 1U && ! tls_ls->resources_released &&
                      fixture.context.tls_output_count == 0 && fixture.context.tls_finish_count == 0 &&
                      fixture.context.events_len == 0,
    ...
    requireClient(cover_snapshot[0] == 0x17 && cover_snapshot[1] == 0x03 && cover_snapshot[2] == 0x03 &&
                      cover_len >= 5U + 22U,
                  "client cover record did not exercise the ACK candidate range");
    memoryZero(cover_snapshot, sizeof(cover_snapshot));

    realityclientHandleUpstreamFinish(fixture.reality, fixture.line);
    requireClient(strcmp(fixture.context.events, "U") == 0, "client cover-delegation fixture did not close cleanly");
    clientFixtureDestroy(&fixture);
}
...
static void testClientRequestedKeyUpdateResponsePrecedesConfirm(void)
{
    ...
    realityclientTunnelDownStreamPayload(
        fixture.reality, fixture.line, clientJoinRecords(&fixture, requested_key_update, ack));
...
static void testClientFragmentedTicketsThenCoalescedAck(void)
{
    ...
    sbuf_t *ticket_flight = clientDrainTlsServerWire(&fixture, "client TLS fixture emitted no ticket flight");
    ...
    {
        realityclientTunnelDownStreamPayload(
            fixture.reality, fixture.line, clientBufferFromBytes(&fixture, first_bytes + i, 1));
        requireClient(lineIsAlive(fixture.line), "client closed on a fragmented genuine ticket record");
    }
    lineReuseBuffer(fixture.line, first_ticket_record);

    realityclient_lstate_t *ls = lineGetState(fixture.line, fixture.reality);
    requireClient(ls->phase == kRealityClientPhaseTls13AwaitAck && ! ls->handoff_ack_authenticated &&
                      ls->s2c_recv_seq == 0 && ls->c2s_send_seq == 1 && fixture.context.events_len == 0 &&
                      bufferstreamIsEmpty(&ls->handoff_stream),
    ...
    realityclientTunnelDownStreamPayload(
        fixture.reality,
        fixture.line,
        clientJoinRecords(&fixture, second_ticket_record, clientBuildHandoffAck(&fixture)));
...
static void testClientAckCoalescedApplicationCompletesHandoff(void)
{
    ...
    sbuf_t       *application        = buildClientInboundRecord(&fixture,
                                                   ...
                                                   1);
    realityclientTunnelDownStreamPayload(fixture.reality, fixture.line, clientJoinRecords(&fixture, ack, application));
...
static void runClientAckTransitionLineDeath(client_ack_death_point_t death_point, const char *expected_events)
{
    client_lifecycle_fixture_t fixture;
    clientFixtureInitialize(&fixture);
    configureClientAwaitAck(&fixture, true);
    ...
    requireClient(! lineIsAlive(fixture.line) && strcmp(fixture.context.events, expected_events) == 0,
...
static void testClientPreAckSilentFailures(void)
{
    ...
    configureClientAwaitAck(&fixture, true);
    realityclientFailAuthenticated(fixture.reality, fixture.line);
    requireClient(strcmp(fixture.context.events, "UD") == 0 && fixture.context.observed_alert == kRealityV2AlertInvalid,
...
static void testClientRequestLineDeath(void)
{
    ...
    requireClient(strcmp(fixture.context.events, "RD") == 0, "client request line-death callback ordering changed");
    clientFixtureDestroy(&fixture);
}
...
static void clientSizingCapture(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    requireClient(record_len >= kRealityV2TlsRecordHeaderSize, "client sizing capture received a truncated TLS record");

    reality_v2_record_descriptor_t descriptor;
    requireClient(realityV2BuildRecordDescriptor(
                      ls->tls_version, &ls->record_profile, kRealityV2RecordKindApplicationData, &descriptor),
                                ...
                                bool kill_after_first_record)
{
    ...
    bufferpoolUpdateAllocationPaddings(pool, kRealityClientMaxFramePrefixSize, kRealityClientMaxFramePrefixSize);
    ...
    requireClient(input_len <= sbufGetMaximumWriteableSize(input), "client sizing input does not fit the test buffer");
    ...
    *(client_sizing_context_t **) tunnelGetState(capture) = &context;
    ...
    {
        requireClient(sent && context.record_count == expected_records && context.recovered_payload_len == input_len,
                      "client sizing sender emitted the wrong chunk sequence");
        if (input_len == 32768)
        {
            requireClient(context.record_count == 2, "client 32768-byte callback emitted a tiny third record");
        }
    }
...
void realityTestClientRecordSizing(void)
{
    ...
    for (size_t i = 0; i < sizeof(profiles) / sizeof(profiles[0]); ++i)
    {
        ...
        {
            runClientSizingCase(profiles[i].tls_version, &profiles[i].profile, input_lengths[j], false);
        }
