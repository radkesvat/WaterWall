...
static uint32_t countCompleteTlsRecords(const uint8_t *bytes, uint32_t len)
{
    ...
    while (offset < len)
    {
        ...
        requireTlsClient(record_len <= len - offset, "TlsClient real flight ended in a partial record body");
        offset += record_len;
        ++count;
    }
    return count;
...
static void fakePayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    if (is_upstream)
    {
        ...
        if (context->append_coalesced_tickets)
        {
            ...
            requireTlsClient(context->coalesced_ticket_server != NULL && context->takeover_tls != NULL &&
            ...
            requireTlsClient(BIO_read(server_wbio, sbufGetMutablePtr(tickets), (int) pending) == (int) pending,
                             "TlsClient coalesced fixture could not drain tickets");
            context->coalesced_ticket_records = countCompleteTlsRecords(sbufGetRawPtr(tickets), (uint32_t) pending);
            bufferstreamPush(&((tlsclient_lstate_t *) lineGetState(l, context->takeover_tls))->takeover_stream,
                             tickets);
        }
    }
...
static void fixtureInitialize(tlsclient_lifecycle_fixture_t *fixture)
{
    ...
    *(tlsclient_lifecycle_context_t **) tunnelGetState(fixture->next) = &fixture->context;
    ...
    requireTlsClient(tlsclientLinestateInitialize(ls, fixture->ssl_ctx, fixture->pool, NULL, 0),
                     "failed to initialize TlsClient lifecycle line state");
...
static void testRetainedDrainStateIsDestroyedOnFinish(void)
{
    ...
    bufferstreamPush(&ls->takeover_stream, makeBytes(&fixture, partial_record, sizeof(partial_record)));
...
static void testPausedShapedFinishIsSynchronous(void)
{
    static const uint8_t          wire_record[] = {SSL3_RT_APPLICATION_DATA, 0x03, 0x03, 0x00, 0x01, 0x5a};
    tlsclient_lifecycle_fixture_t fixture;
    fixtureInitialize(&fixture);
    ...
    ls->tunnel             = fixture.tls;
    ls->line               = fixture.line;
    tlsrecordshapingOutputQueueInitialize(&ls->shaping_output, fixture.pool);
    requireTlsClient(tlsrecordshapingOutputQueuePushMetadata(&ls->shaping_output, 1000),
                     "TlsClient paused Finish fixture could not queue shaping metadata");

    char queue_error[kTlsRecordShapingErrorSize];
    requireTlsClient(tlsrecordshapingOutputQueueFeed(
                         &ls->shaping_output, makeBytes(&fixture, wire_record, sizeof(wire_record)), 0, queue_error) &&
                         tlsrecordshapingOutputQueueFinishFeed(&ls->shaping_output, queue_error) &&
                         ! tlsrecordshapingOutputQueueIsEmpty(&ls->shaping_output),
                     "TlsClient paused Finish fixture could not queue shaped ciphertext");
...
static sbuf_t *makeBytes(tlsclient_lifecycle_fixture_t *fixture, const uint8_t *bytes, uint32_t len)
{
    sbuf_t *buf = len <= bufferpoolGetSmallBufferSize(fixture->pool) ? bufferpoolGetSmallBuffer(fixture->pool)
...
static void testTakeoverAccumulatorRejectsInvalidHeader(void)
{
    ...
    fixtureInitialize(&fixture);
    tlsclient_lstate_t *ls = lineGetState(fixture.line, fixture.tls);
    bufferstreamPush(&ls->takeover_stream, makeBytes(&fixture, invalid_record, sizeof(invalid_record)));
...
static void testDrainModeBypassesTlsInBothDirections(void)
{
    ...
    tlsclientTunnelUpStreamPayload(upstream.tls, upstream.line, makeBytes(&upstream, raw, sizeof(raw)));
    requireTlsClient(upstream.context.saw_payload, "TlsClient drain mode passed owner-formatted bytes to SSL_write");
    ...
    tlsclientTunnelDownStreamPayload(downstream.tls, downstream.line, makeBytes(&downstream, raw, sizeof(raw)));
...
static sbuf_t *drainBioToBuffer(tlsclient_lifecycle_fixture_t *fixture, BIO *bio)
{
    ...
    sbuf_t *buf = pending <= bufferpoolGetSmallBufferSize(fixture->pool) ? bufferpoolGetSmallBuffer(fixture->pool)
                                                                         : bufferpoolGetLargeBuffer(fixture->pool);
 ...
 */
static tlsclient_real_flight_stats_t consumeRealFlightAtEveryByteBoundary(tlsclient_lifecycle_fixture_t *fixture,
                                                                          sbuf_t                        *flight)
{
    ...
    while (offset < flight_len)
    {
        ...
        uint32_t body_len   = ((uint32_t) bytes[offset + 3] << 8U) | (uint32_t) bytes[offset + 4];
        uint32_t record_len = kTlsClientRecordHeaderSize + body_len;
        requireTlsClient(record_len <= flight_len - offset && bytes[offset] == SSL3_RT_APPLICATION_DATA,
                         "TlsClient byte-split fixture expected a complete protected TLS 1.3 record");

        for (uint32_t i = 0; i < record_len; ++i)
        {
            bufferstreamPush(&ls->takeover_stream, makeBytes(fixture, bytes + offset + i, 1));
            sbuf_t *record   = NULL;
            bool    invalid  = false;
            ...
            requireTlsClient(complete && ! invalid && record != NULL && sbufGetLength(record) == record_len &&
                                 memcmp(sbufGetRawPtr(record), bytes + offset, record_len) == 0,
                             "TlsClient changed a byte-split real TLS 1.3 record");
            requireTlsClient(tlsclientTunnelConsumePostHandshakeRecord(fixture->tls, fixture->line, record) ==
                                 kTlsClientPostHandshakeNeedMore,
    ...
    requireTlsClient(bufferstreamIsEmpty(&ls->takeover_stream), "TlsClient left bytes after byte-split real records");
...
static SSL *createTls13Server(SSL_CTX **server_ctx_out, size_t tickets)
{
    ...
    requireTlsClient(server_ctx != NULL && SSL_CTX_set_min_proto_version(server_ctx, TLS1_3_VERSION) == 1 &&
                         ...
                         SSL_CTX_use_PrivateKey_file(server_ctx, REALITY_TEST_KEY_FILE, SSL_FILETYPE_PEM) == 1 &&
...
static void testServerFinishedAndRealTicketsShareOneCallback(void)
{
    ...
    for (size_t ticket_index = 0; ticket_index < sizeof(ticket_counts) / sizeof(ticket_counts[0]); ++ticket_index)
    {
        ...
        requireTlsClient(n < 0 && (server_error == SSL_ERROR_WANT_READ || server_error == SSL_ERROR_WANT_WRITE),
                         "coalesced TlsClient fixture did not produce the server Finished flight");
        sbuf_t *server_finished_flight = drainBioToBuffer(&fixture, SSL_get_wbio(server));
        ...
        tlsclient_real_flight_stats_t stats =
            consumeRealFlightAtEveryByteBoundary(&fixture, fixture.context.pending_raw);
        ...
        requireTlsClient(tlsclientTunnelCompleteTakeover(fixture.tls, fixture.line) && ls->resources_released &&
...
static SSL *enterRealTls13Drain(tlsclient_lifecycle_fixture_t *fixture, SSL_CTX **server_ctx_out)
{
...
static tlsclient_post_handshake_result_t consumeSingleRealRecord(tlsclient_lifecycle_fixture_t *fixture, sbuf_t *record)
{
...
static void testPostHandshakeCloseNotifyClosesDirectly(void)
{
    ...
    requireTlsClient(SSL_shutdown(server) == 0, "TlsClient test peer did not emit TLS 1.3 close_notify");
    tlsclient_post_handshake_result_t result =
        consumeSingleRealRecord(&fixture, drainBioToBuffer(&fixture, SSL_get_wbio(server)));
...
static void testPostHandshakeCorruptionClosesFatally(void)
{
    ...
    requireTlsClient(SSL_write(server, cover_application, sizeof(cover_application)) == (int) sizeof(cover_application),
    ...
    requireTlsClient(consumeSingleRealRecord(&fixture, record) == kTlsClientPostHandshakeFatal,
...
static void testPostHandshakeOutputReentryDestroysStateSafely(void)
{
    ...
    requireTlsClient(SSL_key_update(server, SSL_KEY_UPDATE_REQUESTED) == 1 && SSL_write(server, NULL, 0) == 0,
...
static void testTakeoverDownstreamPathStopsAtRealTls13RecordBoundary(void)
{
    ...
    n = SSL_accept(server);
    requireTlsClient(
        n < 0 && (SSL_get_error(server, n) == SSL_ERROR_WANT_READ || SSL_get_error(server, n) == SSL_ERROR_WANT_WRITE),
    ...
    {
        tlsclientTunnelDownStreamPayload(fixture.tls, fixture.line, makeBytes(&fixture, flight_bytes + i, 1));
        requireTlsClient(lineIsAlive(fixture.line), "TlsClient closed during byte-split real TLS 1.3 handshake");
    }
    ...
    tlsclientTunnelDownStreamPayload(
    ...
    requireTlsClient(ticket_flight != NULL, "TlsClient test server did not emit configured post-handshake tickets");
    fixture.context.saw_payload = false;
    ...
    requireTlsClient(tlsclientTunnelCompleteTakeover(fixture.tls, fixture.line) && ls->resources_released &&
                         ls->ssl == NULL && ls->rbio == NULL && ls->wbio == NULL &&
                         bufferstreamIsEmpty(&ls->takeover_stream) &&
                         ls->takeover_phase == kTlsClientTakeoverPassthrough,
...
static void testPhasedTakeoverApisWithRealTls13(void)
{
    ...
    requireTlsClient(! tlsclientTunnelBeginTakeoverDrain(fixture.tls, fixture.line, &duplicate_begin) &&
    ...
    requireTlsClient(! fixture.context.saw_payload, "TlsClient exposed cover application plaintext during drain");

    requireTlsClient(SSL_key_update(server, SSL_KEY_UPDATE_NOT_REQUESTED) == 1 && SSL_write(server, NULL, 0) == 0,
    ...
    requireTlsClient(SSL_write(server, application_after_key_update, sizeof(application_after_key_update)) ==
                             (int) sizeof(application_after_key_update) &&
                         consumeRawFlight(&fixture, drainBioToBuffer(&fixture, SSL_get_wbio(server))),
                     "TlsClient did not advance read keys for an unrequested KeyUpdate");
    requireTlsClient(! fixture.context.saw_payload, "TlsClient exposed cover plaintext after an unrequested KeyUpdate");
    ...
    requireTlsClient(fixture.context.saw_payload, "TlsClient did not flush the requested KeyUpdate response upstream");
    uint8_t server_plaintext[1];
    int     server_read = SSL_read(server, server_plaintext, sizeof(server_plaintext));
    requireTlsClient(server_read < 0 && SSL_get_error(server, server_read) == SSL_ERROR_WANT_READ,
    ...
    fixture.context.saw_payload          = false;
    tlsclientTunnelUpStreamPayload(fixture.tls, fixture.line, makeBytes(&fixture, owner_raw, sizeof(owner_raw)));
    requireTlsClient(fixture.context.saw_payload && fixture.context.upstream_payload_count == upstream_before + 1U,
                     "TlsClient did not pass owner bytes upstream after CompleteTakeover");
    fixture.context.saw_payload = false;
    tlsclientTunnelDownStreamPayload(fixture.tls, fixture.line, makeBytes(&fixture, owner_raw, sizeof(owner_raw)));
    requireTlsClient(fixture.context.saw_payload && fixture.context.upstream_payload_count == upstream_before + 1U,
...
static void testRealTicketsAndKeyUpdatesAtEveryByteBoundary(void)
{
    ...
    ls->handshake_completed = true;
    sbuf_t *pending_raw     = NULL;
    requireTlsClient(tlsclientTunnelBeginTakeoverDrain(fixture.tls, fixture.line, &pending_raw) && pending_raw == NULL,
                     "TlsClient byte-split fixture did not enter drain mode once");

    tlsclient_real_flight_stats_t ticket_stats = consumeRealFlightAtEveryByteBoundary(&fixture, ticket_tail);
    requireTlsClient(SSL_write(server, NULL, 0) == 0, "TlsClient byte-split fixture could not flush tickets");
    tlsclient_real_flight_stats_t flushed_ticket_stats =
        consumeRealFlightAtEveryByteBoundary(&fixture, drainBioToBuffer(&fixture, SSL_get_wbio(server)));
    ...
    tlsclient_real_flight_stats_t unrequested_stats =
        consumeRealFlightAtEveryByteBoundary(&fixture, drainBioToBuffer(&fixture, SSL_get_wbio(server)));
    requireTlsClient(unrequested_stats.records == 1 && unrequested_stats.split_boundaries > 0 &&
    ...
    requireTlsClient(SSL_write(server, after_unrequested, sizeof(after_unrequested)) == (int) sizeof(after_unrequested),
                     "TlsClient byte-split fixture could not write after KeyUpdate");
    tlsclient_real_flight_stats_t application_stats =
        consumeRealFlightAtEveryByteBoundary(&fixture, drainBioToBuffer(&fixture, SSL_get_wbio(server)));
    requireTlsClient(application_stats.records == 1 && fixture.context.upstream_payload_count == output_before,
    ...
    tlsclient_real_flight_stats_t requested_stats =
        consumeRealFlightAtEveryByteBoundary(&fixture, drainBioToBuffer(&fixture, SSL_get_wbio(server)));
    requireTlsClient(requested_stats.records == 1 && requested_stats.split_boundaries > 0 &&
    ...
    requireTlsClient(read_result < 0 && SSL_get_error(server, read_result) == SSL_ERROR_WANT_READ,
...
int main(void)
{
    ...
    testPausedShapedFinishIsSynchronous();
