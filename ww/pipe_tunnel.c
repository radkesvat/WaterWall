...
static void defaultPipeTunnelUpStreamPayload(tunnel_t *self, line_t *line, shift_buffer_t *payload)
{
    assert(self->up != NULL);
...
static void defaultPipeTunneldownStreamPayload(tunnel_t *self, line_t *line, shift_buffer_t *payload)
{
    assert(self->dw != NULL);
