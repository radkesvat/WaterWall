...
void createWW(const ww_construction_data_t init_data)
{
    ...
    buffer_pools =
        (struct buffer_pool_s **) malloc(sizeof(struct buffer_pool_s *) * (workers_count + kAdditionalReservedWorkers));
    ...
    libhv_hio_pools    = (struct generic_pool_s **) malloc(sizeof(struct generic_pool_s *) *
                                                           (workers_count + kAdditionalReservedWorkers));
    /*
        note that we dont touch additional workers since they must initialize what they want themselves
    */
    for (unsigned int i = 0; i < workers_count; ++i)
    {
        ...
        context_pools[i] = newGenericPoolWithCap((16) + ram_profile, allocContextPoolHandle, destroyContextPoolHandle);
