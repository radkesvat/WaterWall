 ...
 */
WW_EXPORT wtimer_t *wtimerAddPeriod(wloop_t *loop, wtimer_cb cb, int8_t minute DEFAULT(0), int8_t hour DEFAULT(-1),
...
#ifdef EVENT_IOCP
// Native IOCP accounting hooks (used by tests to assert a clean baseline).
WW_EXPORT uint32_t wloopGetIocpLiveOperations(wloop_t *loop);
WW_EXPORT uint32_t wloopGetIocpPostedOperations(wloop_t *loop);
...
WW_EXPORT wio_t *wioCreateSocket(wloop_t *loop, const char *host, int port, wio_type_e type DEFAULT(WIO_TYPE_TCP),
                                 wio_side_e side DEFAULT(WIO_SERVER_SIDE));
WW_EXPORT wio_t *wioCreateSocketWithOptions(wloop_t *loop, const char *host, int port, wio_type_e type, wio_side_e side,
...
// @tcp_client: wioCreateSocket(loop, host, port, WIO_TYPE_TCP, WIO_CLIENT_SIDE) -> wioSetCallBackConnect ->
// wioSetCallBackClose -> wioConnect
// @see examples/nc.c
WW_EXPORT wio_t *wloopCreateTcpClient(wloop_t *loop, const char *host, int port, wconnect_cb connect_cb,
                                      wclose_cb close_cb);
...
WW_EXPORT wio_t *wloopCreateUdpServerWithOptions(wloop_t *loop, const char *host, int port, const char *interface_name,
                                                 int fwmark);
WW_EXPORT wio_t *wloopCreateUdpServerWithBufferOptions(wloop_t *loop, const char *host, int port,
                                                       const char *interface_name, int fwmark, int send_buffer_size,
