 ...
 *
 * fd =>
 * hio_get => EVENTLOOP_ALLOC_SIZEOF(io) => hio_init => hio_ready
 *
 * hio_read  => hio_add(WW_READ) => hio_read_cb
 * hio_write => hio_add(WW_WRITE) => hio_write_cb
 * hio_close => hio_done => hio_del(WW_RDWR) => hio_close_cb
 *
 * hloop_stop => hloop_free => hio_free => EVENTLOOP_FREE(io)
 */
