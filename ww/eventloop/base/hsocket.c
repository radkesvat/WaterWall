...
int wwBind(int port, const char* host, int type) {
#ifdef OS_WIN
    WSAInit();
#endif
...
int wwListen(int port, const char* host) {
    int sockfd = wwBind(port, host, SOCK_STREAM);
    if (sockfd < 0) return sockfd;
...
int wwConnectNonblock(const char* host, int port) {
    return wwConnect(host, port, 1);
}

int wwConnectTimeout(const char* host, int port, int ms) {
...
#ifdef ENABLE_UDS
int wwBindUnix(const char* path, int type) {
    sockaddr_u localaddr;
    memset(&localaddr, 0, sizeof(localaddr));
...
int wwListenUnix(const char* path) {
    int sockfd = wwBindUnix(path, SOCK_STREAM);
    if (sockfd < 0) return sockfd;
    return ListenFD(sockfd);
}

int wwConnectUnix(const char* path, int nonblock) {
    sockaddr_u peeraddr;
    memset(&peeraddr, 0, sizeof(peeraddr));
...
int wwConnectUnixNonblock(const char* path) {
    return wwConnectUnix(path, 1);
}

int wwConnectUnixTimeout(const char* path, int ms) {
    int connfd = wwConnectUnix(path, 1);
    if (connfd < 0) return connfd;
    return ConnectFDTimeout(connfd, ms);
}
#endif
