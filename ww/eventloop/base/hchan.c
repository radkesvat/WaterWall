...
// debugging
  ...
  static void _dlog_chan(const char* fname, const char* fmt, ...) {
    ...
    uint32_t seq = AtomicAddx(&seqnext, 1, memory_order_acquire);
...
// channel lock
  ...
  #define chan_lock_dispose(lock) hmutex_destroy(lock)
  ...
  #define chan_lock_dispose(lock) hhybridmutex_dispose(lock)
...
inline static void* chan_bufptr(Chan* c, uint32_t i) {
...
inline static bool chan_send(Chan* c, void* srcelemptr, bool* nullable closed) {
  ...
  if (AtomicLoad(&c->qlen) < c->qcap) {
    ...
    uint32_t i = AtomicAdd(&c->sendx, 1);
...
inline static bool chan_recv(Chan* c, void* dstelemptr, bool* nullable closed) {
  ...
  if (AtomicLoad(&c->qlen) > 0) {
    ...
    uint32_t i = AtomicAdd(&c->recvx, 1);
    if (i == c->qcap - 1)
...
Chan* nullable ChanOpen(Mem mem, size_t elemsize, uint32_t bufcap) {
...
uint32_t  ChanCap(const Chan* c) { return c->qcap; }
