...
{
#if defined(WCRYPTO_BACKEND_SODIUM) && defined(WCRYPTO_HAS_SODIUM_XCHACHA20POLY1305)
    return wCryptoSodiumXChacha20Poly1305Encrypt(dst, src, src_len, ad, ad_len, nonce, key);
#elif defined(WCRYPTO_BACKEND_OPENSSL) && defined(WCRYPTO_HAS_OPENSSL_XCHACHA20POLY1305)
    return wCryptoOpenSSLXChacha20Poly1305Encrypt(dst, src, src_len, ad, ad_len, nonce, key);
#elif defined(WCRYPTO_BACKEND_SOFTWARE) && defined(WCRYPTO_HAS_SOFTWARE_XCHACHA20POLY1305)
    ...
    return wCryptoSodiumXChacha20Poly1305Encrypt(dst, src, src_len, ad, ad_len, nonce, key);
#elif defined(WCRYPTO_HAS_OPENSSL_XCHACHA20POLY1305)
    return wCryptoOpenSSLXChacha20Poly1305Encrypt(dst, src, src_len, ad, ad_len, nonce, key);
