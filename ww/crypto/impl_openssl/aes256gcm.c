...
bool wCryptoOpenSSLAES256GCMIsAvailable(void)
{
    ...
    const bool  available = cipher != NULL;
    EVP_CIPHER_free(cipher);
    return available;
}

wcrypto_status_t wCryptoOpenSSLAES256GCMEncrypt(unsigned char *dst, size_t dst_capacity, const unsigned char *src,
                                                ...
                                                const unsigned char key[WCRYPTO_AES256GCM_KEY_SIZE])
{
    ...
    if (ctx == NULL || EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) != 1 ||
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, WCRYPTO_AES256GCM_NONCE_SIZE, NULL) != 1 ||
...
wcrypto_status_t wCryptoOpenSSLAES256GCMDecrypt(unsigned char *dst, size_t dst_capacity, const unsigned char *src,
                                                size_t src_len, const unsigned char *ad, size_t ad_len,
                                                const unsigned char nonce[WCRYPTO_AES256GCM_NONCE_SIZE],
                                                const unsigned char key[WCRYPTO_AES256GCM_KEY_SIZE])
{
