...
static void blake2s_compress(wcrypto_software_blake2s_ctx_t *ctx, int last)
{
    ...
    wCryptoZero(v, sizeof(v));
    wCryptoZero(m, sizeof(m));
}

// Initialize the hashing context "ctx" with optional key "key".
//      1 <= outlen <= 32 gives the digest size in bytes.
//      Secret key (also <= 32 bytes) is optional (keylen = 0).
wcrypto_status_t wCryptoSoftwareBlake2sInit(wcrypto_software_blake2s_ctx_t *ctx, size_t outlen,
                                            const unsigned char *key, size_t keylen)
{
    ...
    if (keylen > 0)
    {
        status = wCryptoSoftwareBlake2sUpdate(ctx, key, keylen);
        if (status != kWCryptoOk)
        {
...
wcrypto_status_t wCryptoSoftwareBlake2sUpdate(wcrypto_software_blake2s_ctx_t *ctx, const unsigned char *in,
                                              size_t inlen)
{
    size_t           i;
...
wcrypto_status_t wCryptoSoftwareBlake2sFinal(wcrypto_software_blake2s_ctx_t *ctx, unsigned char *out, size_t outlen)
{
    size_t i;
...
void wCryptoSoftwareBlake2sDestroy(wcrypto_software_blake2s_ctx_t *ctx)
{
    if (ctx != NULL)
        wCryptoZero(ctx, sizeof(*ctx));
}
