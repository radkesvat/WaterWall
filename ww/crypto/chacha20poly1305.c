...
{
#if defined(WCRYPTO_BACKEND_SODIUM) && defined(WCRYPTO_HAS_SODIUM_CHACHA20POLY1305)
    return wCryptoSodiumChacha20Poly1305Encrypt(dst, src, src_len, ad, ad_len, nonce, key);
#elif defined(WCRYPTO_BACKEND_OPENSSL) && defined(WCRYPTO_HAS_OPENSSL_CHACHA20POLY1305)
    return wCryptoOpenSSLChacha20Poly1305Encrypt(dst, src, src_len, ad, ad_len, nonce, key);
#elif defined(WCRYPTO_BACKEND_SOFTWARE) && defined(WCRYPTO_HAS_SOFTWARE_CHACHA20POLY1305)
    ...
    return wCryptoSodiumChacha20Poly1305Encrypt(dst, src, src_len, ad, ad_len, nonce, key);
#elif defined(WCRYPTO_HAS_OPENSSL_CHACHA20POLY1305)
    return wCryptoOpenSSLChacha20Poly1305Encrypt(dst, src, src_len, ad, ad_len, nonce, key);
