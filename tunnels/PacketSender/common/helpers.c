                                       ...
                                       uint32_t src_addr_network)
{
    ...
    packetsenderInitIpv4Header(ipheader, kPacketSenderTcpPacketLength, packetsenderPacketIdFromSeed(seed), IP_PROTO_TCP,
                               src_addr_network, state->dest_addr_network);
                                       ...
                                       uint32_t src_addr_network)
{
    ...
    packetsenderInitIpv4Header(ipheader, kPacketSenderUdpPacketLength, packetsenderPacketIdFromSeed(seed), IP_PROTO_UDP,
                               src_addr_network, state->dest_addr_network);
                                        ...
                                        uint32_t src_addr_network)
{
    ...
    packetsenderInitIpv4Header(ipheader, kPacketSenderIcmpPacketLength, packetsenderPacketIdFromSeed(seed),
                               IP_PROTO_ICMP, src_addr_network, state->dest_addr_network);
                                           ...
                                           uint32_t src_addr_network, uint8_t protocol)
{
    ...
    packetsenderInitIpv4Header(ipheader, kPacketSenderGenericPacketLength, packetsenderPacketIdFromSeed(seed), protocol,
                               src_addr_network, state->dest_addr_network);
...
static void packetsenderBuildPacketForProtocol(const packetsender_tstate_t *state, uint8_t *packet, uint32_t src_addr_host,
                                               uint32_t src_addr_network, uint8_t protocol)
{
    switch (protocol)
    {
...
static bool packetsenderWaitUntilDeadline(packetsender_worker_state_t *slot, uint32_t deadline_ms)
{
    ...
    const uint64_t         elapsed_ms =
        (now_ms >= state->schedule_start_ms) ? (now_ms - state->schedule_start_ms) : 0;
...
static void packetsenderSendReadyPackets(packetsender_worker_state_t *slot)
{
    ...
    while (slot->next_packet_index < slot->packet_index_end)
    {
        ...
        packetsenderResolvePacketView(state, current_index, &packet_ptr, &packet_len, &src_addr_network, &src_addr_host,
                                      &protocol);
