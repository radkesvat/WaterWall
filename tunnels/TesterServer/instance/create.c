...
static bool testerserverLoadPacketIpv4Settings(testerserver_tstate_t *ts, const cJSON *settings)
{
     ...
     */
    if (ts->packet_ipv4_worker_affine_flow && ts->packet_ipv4_transport != kTesterServerPacketIpv4TransportTcp &&
        ts->packet_ipv4_transport != kTesterServerPacketIpv4TransportUdp)
    {
        LOGF("TesterServer: settings->packet-ipv4->worker-affine-flow requires transport tcp or udp");
        return false;
    }
...
tunnel_t *testerserverTunnelCreate(node_t *node)
{
    ...
    getIntFromJsonObjectOrDefault(
        &split_payload_burst, settings, "split-payload-burst", kTesterServerSplitPayloadBurst);
