...
static uint32_t obfuscatorserverGetSkipLength(tunnel_t *t, sbuf_t *buf)
{
...
void obfuscatorserverApplyXor(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    discard l;

    obfuscatorserver_tstate_t *ts          = tunnelGetState(t);
    uint32_t                   payload_len = sbufGetLength(buf);
    uint32_t                   skip_length = obfuscatorserverGetSkipLength(t, buf);
    ...
    obfuscatorserverXorByte(sbufGetMutablePtr(buf) + skip_length, payload_len - skip_length, ts->xor_key);
}

static sbuf_t *obfuscatorserverCloneBufferWithPadding(line_t *l, sbuf_t *buf)
{
...
bool obfuscatorserverWrapTlsRecordHeader(line_t *l, sbuf_t **buf_io)
{
    sbuf_t  *buf         = *buf_io;
    uint32_t payload_len = sbufGetLength(buf);
    ...
    {
        LOGW("ObfuscatorServer: payload length %u exceeds TLS record header limit, dropped", payload_len);
        lineReuseBuffer(l, buf);
        return false;
    }
    ...
    {
        buf = obfuscatorserverCloneBufferWithPadding(l, buf);
    }
...
bool obfuscatorserverStripTlsRecordHeader(line_t *l, sbuf_t *buf)
{
    if (sbufGetLength(buf) < kObfuscatorTlsRecordHeaderSize)
    {
        LOGW("ObfuscatorServer: truncated TLS-like payload (%u bytes), dropped", sbufGetLength(buf));
        lineReuseBuffer(l, buf);
        return false;
    }
    ...
    if (record[0] != kObfuscatorTlsApplicationDataType || record[1] != kObfuscatorTlsLegacyVersionMajor ||
        record[2] != kObfuscatorTlsLegacyVersionMinor)
    {
        LOGW("ObfuscatorServer: invalid TLS-like record header, dropped");
        lineReuseBuffer(l, buf);
        return false;
    }
    ...
    {
        LOGW(
            "ObfuscatorServer: TLS-like record length mismatch (%u != %u), dropped", payload_len, expected_payload_len);
        lineReuseBuffer(l, buf);
        return false;
    }
