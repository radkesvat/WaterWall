...
enum
{
    ...
    kPingClientReuseTrailerMagic0 = 0x57,
    kPingClientReuseTrailerMagic1 = 0x50
};
...
static bool pingclientIpv4PacketIsFragmented(const struct ip_hdr *ipheader)
{
    return (lwip_ntohs(IPH_OFFSET(ipheader)) & kIpv4FragmentMask) != 0;
}

static void pingclientRecalculateIpv4HeaderChecksum(struct ip_hdr *ipheader)
{
...
static bool pingclientCheckPacketLengthLimit(const char *operation, uint32_t packet_len)
{
    if (UNLIKELY(packet_len > kMaxAllowedPacketLength))
    {
        LOGW("PingClient: dropping packet because %s would exceed kMaxAllowedPacketLength: %u > %u", operation,
...
static bool pingclientChooseRoundupPayloadLength(uint16_t payload_with_size_len, uint16_t max_payload_len,
                                                 uint16_t *roundup_payload_len_out)
{
    static const uint16_t kRoundupBuckets[] = {56, 128, 256, 512, 1024};
...
static void pingclientFillIcmpEchoHeader(pingclient_tstate_t *state, struct icmp_echo_hdr *icmpheader)
{
...
static void pingclientWriteReuseTrailer(uint8_t *trailer, uint8_t original_protocol, uint16_t transport_len)
{
    trailer[0] = kPingClientReuseTrailerMagic0;
    trailer[1] = kPingClientReuseTrailerMagic1;
    trailer[2] = original_protocol;
    trailer[3] = (uint8_t) ((transport_len >> 8) & 0xFFU);
    trailer[4] = (uint8_t) (transport_len & 0xFFU);
}

static bool pingclientReadReuseTrailer(const uint8_t *icmp_payload, uint16_t icmp_payload_len,
                                       uint8_t *original_protocol_out, uint16_t *transport_len_out)
{
    if (UNLIKELY(icmp_payload_len < kPingClientReuseTrailerLength))
    {
        return false;
    }

    const uint8_t *trailer = icmp_payload + icmp_payload_len - kPingClientReuseTrailerLength;
    if (UNLIKELY(trailer[0] != kPingClientReuseTrailerMagic0 || trailer[1] != kPingClientReuseTrailerMagic1))
    {
        return false;
    }
                                                   ...
                                                   uint32_t *inner_source_addr_out, uint32_t *inner_dest_addr_out)
{
    ...
    *inner_dest_addr_out                = inner_ipheader->dest.addr;
    ...
    {
        if (UNLIKELY(inner_packet_len > kPingClientMaxIcmpPayloadLength - kPingClientSizePrefixLength))
        {
        ...
        if (! pingclientChooseRoundupPayloadLength(payload_with_size_len, kPingClientMaxIcmpPayloadLength,
                                                   &icmp_payload_len))
        {
    ...
    else if (UNLIKELY(inner_packet_len > kPingClientMaxIcmpPayloadLength))
    {
...
static sbuf_t *pingclientPrepareRawIcmpPayloadBuffer(tunnel_t *t, line_t *l, sbuf_t *buf,
                                                     uint16_t *icmp_payload_len_out)
{
    pingclient_tstate_t *state   = tunnelGetState(t);
    ...
    {
        uint16_t packet_len;
        if (pingclientValidateIpv4PacketBytes(sbufGetRawPtr(buf), raw_len, &packet_len))
        {
            calcFullPacketChecksum(sbufGetMutablePtr(buf));
        }
        lineSetRecalculateChecksum(l, false);
    ...
    {
        LOGW("PingClient: dropping raw payload because it is too large for an ICMP frame: %u",
    ...
    if (state->roundup_payload_size)
    {
        if (UNLIKELY(raw_len > kPingClientMaxOnlyIcmpPayloadLength - kPingClientSizePrefixLength))
        {
            LOGW("PingClient: dropping raw payload because roundup-size needs %u bytes but ICMP payload is capped at %u",
                 (unsigned int) (raw_len + kPingClientSizePrefixLength),
                 (unsigned int) kPingClientMaxOnlyIcmpPayloadLength);
            return NULL;
        }

        const uint16_t payload_with_size_len = (uint16_t) (raw_len + kPingClientSizePrefixLength);
        if (! pingclientChooseRoundupPayloadLength(payload_with_size_len, kPingClientMaxOnlyIcmpPayloadLength,
                                                   &icmp_payload_len))
        {
            LOGW("PingClient: dropping raw payload because roundup-size cannot fit %u bytes inside the ICMP payload limit",
                 (unsigned int) payload_with_size_len);
            return NULL;
        }
        ...
        uint8_t *payload = sbufGetMutablePtr(buf);
        memoryMove(payload + kPingClientSizePrefixLength, payload, raw_len);
    ...
    else if (UNLIKELY(raw_len > kPingClientMaxOnlyIcmpPayloadLength))
    {
        LOGW("PingClient: raw payload exceeds ICMP payload size limit: %u > %u", (unsigned int) raw_len,
             (unsigned int) kPingClientMaxOnlyIcmpPayloadLength);
        return NULL;
    }
...
{
    if (UNLIKELY(sbufGetLength(buf) < kPingClientEncapsulationOverhead))
    {
        return false;
    }
    ...
    if (UNLIKELY(sbufGetLength(buf) < (uint32_t) ip_header_len + kPingClientIcmpHeaderLength))
    {
        return false;
    }
    ...
    if (UNLIKELY(pingclientIpv4PacketIsFragmented(ipheader)))
    {
        return false;
    }
...
static bool pingclientMatchOnlyIcmpEnvelope(const pingclient_tstate_t *state, sbuf_t *buf)
{
    if (UNLIKELY(sbufGetLength(buf) < kPingClientIcmpHeaderLength ||
...
static void pingclientEncapsulateWithNewIpAndIcmp(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    const uint32_t packet_len = (uint32_t) icmp_payload_len + kPingClientEncapsulationOverhead;
    if (! pingclientCheckPacketLengthLimit("IPv4+ICMP encapsulation", packet_len))
    {
        lineReuseBuffer(l, buf);
        return;
    }

    if (UNLIKELY(sbufGetLeftCapacity(buf) < kPingClientEncapsulationOverhead))
    {
    ...
    memoryZero(packet, kPingClientEncapsulationOverhead);
...
static void pingclientEncapsulateReusingIpv4Header(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    pingclient_tstate_t *state = tunnelGetState(t);
    uint16_t             packet_len;
    ...
    const uint16_t max_icmp_payload_len =
        (uint16_t) (kMaxAllowedPacketLength - ip_header_len - kPingClientIcmpHeaderLength);
    uint16_t icmp_payload_len = (uint16_t) (transport_len + kPingClientReuseTrailerLength);
    ...
    uint8_t              *icmp_payload = packet + ip_header_len + kPingClientIcmpHeaderLength;
    uint8_t              *transport    = packet + ip_header_len;

    memoryMove(icmp_payload, transport, transport_len);

    const uint16_t padding_len = (uint16_t) (icmp_payload_len - transport_len - kPingClientReuseTrailerLength);
    if (padding_len > 0)
    {
        getRandomBytes(icmp_payload + transport_len, padding_len);
    }

    pingclientWriteReuseTrailer(icmp_payload + icmp_payload_len - kPingClientReuseTrailerLength, original_protocol,
                                transport_len);
    ...
    memoryZero(icmpheader, kPingClientIcmpHeaderLength);
    pingclientFillIcmpEchoHeader(state, icmpheader);
...
static void pingclientEncapsulateOnlyIcmp(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    pingclient_tstate_t *state = tunnelGetState(t);
    uint16_t             icmp_payload_len;
    ...
    const uint32_t frame_len = (uint32_t) icmp_payload_len + kPingClientIcmpHeaderLength;
    if (! pingclientCheckPacketLengthLimit("ICMP-only encapsulation", frame_len))
    {
        lineReuseBuffer(l, buf);
        return;
    }

    if (UNLIKELY(sbufGetLeftCapacity(buf) < kPingClientIcmpHeaderLength))
    {
        LOGW("PingClient: dropping raw payload without enough left padding for ICMP-only encapsulation");
        lineReuseBuffer(l, buf);
        return;
    }

    sbufShiftLeft(buf, kPingClientIcmpHeaderLength);
...
static void pingclientForwardDecodedDownstream(tunnel_t *t, line_t *l, sbuf_t *buf, uint16_t header_len,
                                               uint16_t payload_len_after_strip, bool has_size_prefix)
{
    ...
    if (has_size_prefix)
    {
        memoryMove(sbufGetMutablePtr(buf), sbufGetMutablePtr(buf) + kPingClientSizePrefixLength,
...
static void pingclientDecapsulateNewIpAndIcmp(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    pingclient_tstate_t *state = tunnelGetState(t);
    uint16_t             outer_header_len;

    if (! pingclientMatchIpv4IcmpEnvelope(state, buf, &outer_header_len))
    {
...
static void pingclientDecapsulateOnlyIcmp(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    pingclient_tstate_t *state = tunnelGetState(t);

    if (! pingclientMatchOnlyIcmpEnvelope(state, buf))
    {
        tunnelPrevDownStreamPayload(t, l, buf);
        return;
    }
    ...
    pingclientForwardDecodedDownstream(t, l, buf, kPingClientIcmpHeaderLength, raw_payload_len, has_size_prefix);
}
...
static void pingclientSwapIpv4ProtocolToIcmp(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    {
        IPH_PROTO_SET(ipheader, IP_PROTO_ICMP);
        pingclientRecalculateIpv4HeaderChecksum(ipheader);
        lineSetRecalculateChecksum(l, false);
    }
...
void pingclientEncapsulatePacket(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    {
    case kPingClientStrategyWrapNewIpAndIcmpHeader:
        pingclientEncapsulateWithNewIpAndIcmp(t, l, buf);
        return;
    case kPingClientStrategyWrapIcmpHeaderAndReuseIpv4Addrs:
        pingclientEncapsulateReusingIpv4Header(t, l, buf);
        return;
    case kPingClientStrategyWrapOnlyIcmpHeader:
        pingclientEncapsulateOnlyIcmp(t, l, buf);
        return;
    case kPingClientStrategyChangeOnlyIpv4ProtocolNumber:
        pingclientSwapIpv4ProtocolToIcmp(t, l, buf);
        return;
    default:
        LOGW("PingClient: dropping packet because strategy %u is invalid", (unsigned int) state->strategy);
        lineReuseBuffer(l, buf);
        return;
    }
}
...
void pingclientDecapsulatePacket(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    {
    case kPingClientStrategyWrapNewIpAndIcmpHeader:
        pingclientDecapsulateNewIpAndIcmp(t, l, buf);
        return;
    case kPingClientStrategyWrapIcmpHeaderAndReuseIpv4Addrs:
        pingclientDecapsulateReusedIpv4Header(t, l, buf);
        return;
    case kPingClientStrategyWrapOnlyIcmpHeader:
        pingclientDecapsulateOnlyIcmp(t, l, buf);
        return;
    case kPingClientStrategyChangeOnlyIpv4ProtocolNumber:
