...
static sbuf_t *httpserverV2GetNgHttp2SendableData(httpserver_lstate_t *ls)
{
    char    *buf = NULL;
                                        ...
                                        size_t len, void *userdata)
{
    ...
    contextqueuePush(&ls->cq_u, contextCreatePayload(ls->line, buf));
...
int httpserverV2OnFrameRecvCallBack(nghttp2_session *session, const nghttp2_frame *frame, void *userdata)
{
    ...
    if ((frame->hd.type & NGHTTP2_HEADERS) == NGHTTP2_HEADERS)
    {
        ...
        if (frame->headers.cat == NGHTTP2_HCAT_REQUEST)
        {
            ...
            nghttp2_submit_headers(ls->session, flags, frame->hd.stream_id, NULL, &nvs[0], nvlen, NULL);
...
int httpserverV2OnStreamClosedCallBack(nghttp2_session *session, int32_t stream_id, uint32_t error_code, void *userdata)
{
    discard error_code;
    discard userdata;
    ...
    contextqueuePush(&ls->cq_u, contextCreateFin(ls->line));
    return 0;
}
