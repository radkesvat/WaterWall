...
static void on_recv(hio_t *io, void *buf, int readbytes) {
  connector_con_state_t *cstate =
      (connector_con_state_t *)(hevent_userdata(io));
...
void connectorPacketUpStream(tunnel_t *self, context_t *c) {
  ...
  if (c->payload != NULL) {
    ...
    if (c->init) {
      ...
      // fill the resolved_dest_context address based on settings
      {
        ...
        if (state->dest_addr.status == cdvs_from_source)
          ...
          if (state->dest_atype == SAT_DOMAINNAME) {
            ...
            sockaddr_set_ip(&(resolved_dest_context.addr),
                            state->dest_addr.value_ptr);
        }

        if (state->dest_port.status == cdvs_from_source)
          sockaddr_set_port(&(resolved_dest_context.addr),
                            sockaddr_port(&(src_ctx->addr)));
        else if (state->dest_port.status == cdvs_from_dest)
          sockaddr_set_port(&(resolved_dest_context.addr),
                            sockaddr_port(&(dest_ctx->addr)));
        else
          sockaddr_set_port(&(resolved_dest_context.addr),
      ...
      hio_set_peeraddr(cstate->io, &(resolved_dest_context.addr.sa),
                       sockaddr_len(&(resolved_dest_context.addr)));
