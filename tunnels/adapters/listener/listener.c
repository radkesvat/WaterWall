...
tunnel_t *newListener(node_instance_context_t *instance_info)
{
    ...
    memorySet(state, 0, sizeof(listener_state_t));
    ...
    udp_inbound_node->name    = stringConcat(instance_info->node->name, "_udp_inbound");
    udp_inbound_node->type    = stringDuplicate("UdpListener");
