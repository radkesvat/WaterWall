...
void protoswaptrickUpStreamPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    if (IPH_V(ipheader) == 4)
    {
        if (IPH_PROTO(ipheader) == IPPROTO_TCP && state->trick_proto_swap_tcp_number != -1)
        {
            IPH_PROTO_SET(ipheader, state->trick_proto_swap_tcp_number);
            l->recalculate_checksum = true;
        }
        else if (IPH_PROTO(ipheader) == IPPROTO_UDP && state->trick_proto_swap_udp_number != -1)
        {
...
void protoswaptrickDownStreamPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    if (IPH_V(ipheader) == 4)
    {
        if (state->trick_proto_swap_tcp_number != -1 && IPH_PROTO(ipheader) == state->trick_proto_swap_tcp_number)
        {
            IPH_PROTO_SET(ipheader, IPPROTO_TCP);
            l->recalculate_checksum = true;
        }
        else if (state->trick_proto_swap_udp_number != -1 && IPH_PROTO(ipheader) == state->trick_proto_swap_udp_number)
        {
