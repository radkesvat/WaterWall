...
void httpproxyclientTunnelOnChain(tunnel_t *t, tunnel_chain_t *chain)
{
    ...
    if (t->prev == NULL)
    {
        ...
        {
            LOGF("HttpProxyClient: cannot defer chaining without a previous tunnel on a non-empty chain");
    ...
    if (node->hash_next == 0)
    {
        LOGF("HttpProxyClient: a next node is required");
        startupFailureRecord(1);
        return;
    }
    ...
    if (next_tunnel == NULL)
    {
        LOGF("HttpProxyClient: next node \"%s\" has no tunnel instance", next_node->name);
        startupFailureRecord(1);
        return;
    }
    if (resolver != NULL && (resolver->prev != NULL || resolver->next != NULL))
    {
        LOGF("HttpProxyClient: internal DomainResolver tunnel is already bound");
        startupFailureRecord(1);
        return;
    }
