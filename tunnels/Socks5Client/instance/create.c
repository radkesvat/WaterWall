...
static bool socks5clientCreateInternalDomainResolver(tunnel_t *t, node_t *node)
{
    ...
    ts->domain_resolver_settings = socks5clientCreateDomainResolverSettings((enum domain_strategy) ts->domain_strategy);
    ...
    domainresolverTunnelSetPrepareHook(ts->domain_resolver_tunnel, t, 0, socks5clientDomainResolverPrepare, NULL);
...
static bool parseCredentials(socks5client_tstate_t *ts, const cJSON *settings)
{
    ...
    getStringFromJsonObjectByKeys(&user, settings, "user", "username", NULL);
    getStringFromJsonObjectByKeys(&pass, settings, "pass", "password", NULL);
